...
import type {
  ...
  DirectActionRequest,
  DirectActionRequestEnvelope,
  DirectActionResultEnvelope,
  DirectTerminalEnvelope,
...
function getDirectBatchStageActionTypes(session: AgentSessionRecord): readonly DirectActionRequest['type'][] | null {
  ...
  if (session.mode === 'plan' && (session.role === 'planner' || session.role === 'plan-reviewer' || session.role === 'final-reviewer')) {
    return DIRECT_BATCH_READ_ONLY_ACTION_TYPES;
  }
  if (session.mode === 'agent' && session.role === 'worker') {
    return DIRECT_BATCH_WORKER_ACTION_TYPES;
  }
  return null;
}

function getDirectActionTypesForSession(session: AgentSessionRecord): readonly DirectActionRequest['type'][] | null {
  if (session.type === 'batch_analysis') {
    return getDirectBatchStageActionTypes(session);
  }
  if (session.type === 'article_edit') {
    return DIRECT_ARTICLE_EDIT_ACTION_TYPES;
  }
...
export class CursorAcpRuntime {
  ...
  async checkHealth(workspaceId: string, selectedMode: KbAccessMode = DEFAULT_AGENT_ACCESS_MODE, workspaceKbAccessMode?: KbAccessMode): Promise<AgentHealthCheckResponse> {
    ...
    const availableModes = KB_ACCESS_MODES
  ...
  private shouldUseDirectBatchActionLoop(session: AgentSessionRecord): boolean {
    return (session.kbAccessMode ?? DEFAULT_AGENT_ACCESS_MODE) === 'direct'
      && Boolean(getDirectBatchStageActionTypes(session));
  }

  private shouldUseDirectActionLoop(session: AgentSessionRecord): boolean {
    return (session.kbAccessMode ?? DEFAULT_AGENT_ACCESS_MODE) === 'direct'
      && Boolean(getDirectActionTypesForSession(session));
  }

  private async runDirectActionLoop(
    session: AgentSessionRecord,
    taskPayload: Record<string, unknown>,
    emit: (payload: Omit<AgentStreamingPayload, 'sessionId' | 'atUtc'>) => Promise<void> | void,
    toolCalls: AgentRunResult['toolCalls'],
    rawOutput: string[],
    isCancelled: () => boolean,
    timeoutMs: number,
    startedAtMs: number
  ): Promise<{ resultPayload: unknown; finalText?: string; terminalEnvelope?: DirectTerminalEnvelope }> {
    ...
    let turnCount = 0;
    let recoveryCount = 0;
    const repeatedActionDigests = new Map<string, number>();
    ...
    while (true) {
      ...
      const settleWindow = session.type === 'batch_analysis'
        ? getBatchAnalysisPromptSettleWindow(
      ...
      if (turnCount > DIRECT_ACTION_LOOP_MAX_TURNS) {
        const blocked = buildDirectBlockedResultEnvelope(
          `Direct action loop exceeded ${DIRECT_ACTION_LOOP_MAX_TURNS} turns.`
        );
        return {
          resultPayload: buildSyntheticPromptResult(blocked),
          finalText: JSON.stringify(blocked),
      ...
      if (repeatCount > DIRECT_ACTION_REPEAT_LIMIT) {
        const blocked = buildDirectBlockedResultEnvelope(
          `Direct action loop repeated the same ${actionEnvelope.action.type} request too many times.`,
          {
      ...
      await this.appendTranscriptLine(
        ...
        JSON.stringify(actionEnvelope.action)
      );
  ...
  async runBatchAnalysis(
    ...
    try {
      ...
      const streamEmit = (event: Omit<AgentStreamingPayload, 'sessionId' | 'atUtc'>) => {
      ...
      if (this.shouldUseDirectBatchActionLoop(session)) {
        const directLoopResult = await this.runDirectActionLoop(
          session,
          {
            ...baseTaskPayload,
  ...
  async runArticleEdit(
    ...
    try {
      ...
      const directLoopResult = this.shouldUseDirectActionLoop(session)
        ? await this.runDirectActionLoop(
            session,
            taskPayload,
            streamEmit,
            toolCalls,
  ...
  async runAssistantChat(
    ...
    try {
      ...
      while (true) {
        let directLoopResult:
          | { resultPayload: unknown; finalText?: string; terminalEnvelope?: DirectTerminalEnvelope }
          | null = null;
        let transitResultPayload: unknown = undefined;
        try {
          const taskPayload = {
            task: 'assistant_chat',
            localeVariantId: request.localeVariantId,
          ...
          if (useDirectActionLoop) {
            directLoopResult = await this.runDirectActionLoop(
              session,
              taskPayload,
              streamEmit,
              toolCalls,
            ...
            transitResultPayload = await this.transit(
              session,
              taskPayload,
              streamEmit,
              toolCalls,
  ...
  private async resolveSession(input: AgentRunRequestWithReusePolicy): Promise<AgentSessionRecord> {
    ...
    if (!session) {
      ...
      const createRequest: AgentSessionCreateRequest = {
        ...
        scope:
          'localeVariantScope' in input && input.localeVariantScope
  ...
  private getProvider(mode: KbAccessMode): KbAccessProvider {
    if (mode === 'direct') {
      return {
        ...
        getHealth: (workspaceId?: string) => this.getDirectHealth(workspaceId)
      };
    }
  ...
  private async getDirectHealth(workspaceId?: string): Promise<KbAccessHealth> {
    ...
    const issues = new Set<string>(executorHealth.issues ?? []);
...
function extractDirectProtocolObject(result: unknown): Record<string, unknown> | null {
  const streamedText =
    result && typeof result === 'object' && typeof (result as { streamedText?: unknown }).streamedText === 'string'
      ? String((result as { streamedText: string }).streamedText)
      : '';
...
function parseDirectActionEnvelope(result: unknown): DirectActionRequestEnvelope | null {
  ...
  if (
    ...
    || !DIRECT_ACTION_TYPES.includes(request.type as (typeof DIRECT_ACTION_TYPES)[number])
    || !request.args
  ...
  return {
    ...
    action: {
      id: request.id,
      type: request.type as DirectActionRequest['type'],
      args: request.args as DirectActionRequest['args']
    } as DirectActionRequest
  };
}

function parseDirectTerminalEnvelope(result: unknown): DirectTerminalEnvelope | null {
  const parsed = extractDirectProtocolObject(result);
  if (!parsed || parsed.isFinal !== true) {
    return null;
  }
  if (
    parsed.completionState !== 'blocked'
...
function compactDirectActionPromptValue(value: unknown, depth = 0): unknown {
  ...
  if (typeof value === 'object') {
    ...
    const compacted = Object.fromEntries(
      limitedEntries.map(([key, entry]) => [key, compactDirectActionPromptValue(entry, depth + 1)])
...
function serializeDirectActionResultForPrompt(actionResult: DirectActionResultEnvelope): string {
  const serialized = JSON.stringify(actionResult);
  if (serialized.length <= DIRECT_ACTION_RESULT_MAX_PROMPT_CHARS) {
    return serialized;
  }

  const compacted = compactDirectActionPromptValue(actionResult) as Record<string, unknown>;
...
function stripDirectContinuation(prompt: string | undefined): string {
  const normalized = prompt?.trim() ?? '';
  if (!normalized) {
    return '';
  }
...
function buildDirectContinuationPrompt(
  session: AgentSessionRecord,
  prompt: string,
  actionResult: DirectActionResultEnvelope,
  turnCount: number
): string {
...
function buildDirectRecoveryPrompt(
  session: AgentSessionRecord,
  prompt: string,
...
function looksLikeRecoverableDirectProtocolText(value: string | undefined): boolean {
  const trimmed = value?.trim() ?? '';
  if (!trimmed) {
    return false;
  }
  ...
  return (
    normalized.includes('"completionstate"')
...
function buildDirectBlockedResultEnvelope(message: string, details?: unknown): DirectTerminalEnvelope {
  return {
...
function buildSyntheticPromptResult(payload: Record<string, unknown> | DirectTerminalEnvelope): Record<string, unknown> {
