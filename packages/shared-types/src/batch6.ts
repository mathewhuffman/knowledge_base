...
export type BatchAnalysisStageStatus =
  | 'queued'
  | 'planning'
  | 'plan_reviewing'
  | 'plan_revision'
  ...
  | 'failed'
  | 'canceled';
export type BatchAnalysisArtifactVerdict =
  | 'approved'
  | 'needs_revision'
  | 'needs_user_input'
  | 'needs_rework'
...
export type BatchAnalysisIterationStatus =
  | 'running'
  | 'completed'
  | 'failed'
  | 'needs_user_input'
...
export function normalizeBatchAnalysisWorkerStageBudgetMinutes(value: unknown): number | undefined {
  ...
  return Math.min(
    ...
    Math.max(MIN_BATCH_ANALYSIS_WORKER_STAGE_BUDGET_MINUTES, Math.round(parsed))
  );
}
...
const MCP_NON_EMPTY_STRING_SCHEMA = { type: 'string', minLength: 1 } as const;
