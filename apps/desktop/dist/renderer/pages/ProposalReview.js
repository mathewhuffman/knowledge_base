...
import { ProposalReviewStatus, ProposalReviewDecision, ProposalAction, } from '@kb-vault/shared-types';
...
import { IconCheckCircle, IconChevronLeft, IconChevronRight, IconArchive, IconMapPin, IconGitBranch, IconEye, IconCode, IconFileText, } from '../components/icons';
...
function RenderedDiffPanel({ blocks }) {
    if (!blocks || blocks.length === 0) {
        return (_jsx("div", { style: { padding: 'var(--space-4)', textAlign: 'center', color: 'var(--color-text-muted)', fontSize: 'var(--text-sm)' }, children: "No rendered diff available" }));
    }
    return (_jsx("div", { style: { display: 'flex', flexDirection: 'column' }, children: blocks.map((block, i) => (_jsxs("div", { className: `rendered-diff-block rendered-diff-block--${block.kind}`, children: [block.kind === 'removed' && block.beforeText, block.kind === 'added' && block.afterText, block.kind === 'unchanged' && (block.beforeText || block.afterText)] }, i))) }));
}
...
function ProposalBatchRow({ batch, onOpen, }) {
    ...
    return (_jsxs("tr", { className: "proposal-batch-table-row", onClick: onOpen, onKeyDown: (event) => {
            if (event.key === 'Enter' || event.key === ' ') {
                event.preventDefault();
                onOpen();
            }
        }, role: "button", tabIndex: 0, children: [_jsxs("td", { style: { fontWeight: 'var(--weight-medium)' }, children: [_jsx("div", { children: batch.batchName }), _jsx("div", { className: "proposal-batch-secondary", children: batch.sourceFileName })] }), _jsx("td", { style: { color: 'var(--color-text-secondary)' }, children: formatDate(batch.importedAtUtc) }), _jsx("td", { children: batch.proposalCount }), _jsx("td", { children: _jsxs("div", { className: "proposal-batch-progress", children: [_jsxs("span", { children: [reviewedCount, " / ", batch.proposalCount] }), _jsx("div", { className: "progress-bar proposal-batch-progress-bar", children: _jsx("div", { className: "progress-bar-fill", style: { width: `${progress}%` } }) })] }) }), _jsx("td", { children: batch.pendingReviewCount > 0 ? (_jsxs("span", { children: [batch.pendingReviewCount, " pending"] })) : (_jsx("span", { className: "proposal-batch-empty-note", children: "All reviewed" })) }), _jsx("td", { children: _jsx(Badge, { variant: batchStatusVariant(batch.batchStatus), children: BATCH_STATUS_LABEL[batch.batchStatus] ?? batch.batchStatus }) }), _jsx("td", { className: "proposal-batch-actions-cell", children: _jsx("button", { className: "btn btn-primary btn-xs", onClick: (event) => {
                        event.stopPropagation();
                        onOpen();
                    }, children: batch.pendingReviewCount > 0 ? 'Review' : 'Open' }) })] }));
}
