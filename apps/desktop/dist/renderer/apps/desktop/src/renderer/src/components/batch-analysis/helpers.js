...
function summarizeSkippedStage(skippedStage, previousStage, nextStage) {
...
function collectSkippedStageEntries(entries, stageEvents = []) {
    ...
    return Array.from(new Map([...stageEventSkippedEntries, ...timelineSkippedEntries].map((entry) => [
        `${entry.iterationId ?? 'none'}:${entry.skippedFromStage}:${entry.stage}:${entry.skippedToStage}`,
        entry,
    ])).values());
}
export function deriveSkippedStages(entries, stageEvents = []) {
    const actualStagesSeen = new Set();
