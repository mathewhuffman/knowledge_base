...
import { IconFolder, IconFolderOpen, IconFileText, IconSearch, IconRefreshCw, IconClock, IconGlobe, IconEye, IconCode, IconLink, IconImage, IconChevronRight, IconChevronDown, IconChevronLeft, IconZap, IconSend, IconCheckCircle, } from '../components/icons';
...
function getExpandedFolderStorageKey(workspaceId) {
    return `${EXPANDED_FOLDER_STORAGE_KEY_PREFIX}:${workspaceId}`;
}
function readExpandedFolderPreference(workspaceId) {
    ...
    try {
        ...
        const folderIds = parsed.filter((value) => typeof value === 'string');
...
function writeExpandedFolderPreference(workspaceId, expandedFolders) {
    if (typeof window === 'undefined') {
        return;
    }
    try {
        window.localStorage.setItem(getExpandedFolderStorageKey(workspaceId), JSON.stringify(Array.from(expandedFolders).sort()));
    }
    catch {
...
export const ArticleExplorer = () => {
    ...
    const createDraftBranchMutation = useIpcMutation('draft.branch.create');
    const saveDraftBranchMutation = useIpcMutation('draft.branch.save');
    ...
    const [articleSurfaceMode, setArticleSurfaceMode] = useState('preview');
    const [articleEditorHtml, setArticleEditorHtml] = useState('');
    const [savedArticleHtml, setSavedArticleHtml] = useState('');
    const [articleEditBranchId, setArticleEditBranchId] = useState(null);
    const [articleSaveMessage, setArticleSaveMessage] = useState(null);
    ...
    const openableTree = useMemo(() => (tree.filter((node) => node.locales.some((locale) => Boolean(locale.revision?.revisionId)))), [tree]);
    ...
    const topLevelFolderIds = useMemo(() => (allFolderTree
        .filter((item) => item.type === 'folder')
        .map((item) => item.id)), [allFolderTree]);
    ...
    const handleSaveArticleEdit = useCallback(async () => {
        if (!activeWorkspace || !detailPanel.detail)
            return;
        let branchId = articleEditBranchId;
        let expectedHeadRevisionId;
        if (!branchId) {
        ...
        setDetailPanel((current) => {
            ...
            return {
                ...current,
                localeVariants: current.localeVariants.map((locale) => (locale.localeVariantId === current.localeVariantId
                    ? {
                        ...locale,
                        revision: {
                            ...locale.revision,
    ...
    const renderDetailContent = () => {
        ...
        return (_jsxs(_Fragment, { children: [detailPanel.activeTab === 'preview' && ((detailPanel.detail.sourceHtml || detailPanel.detail.previewHtml) ? (_jsxs(_Fragment, { children: [_jsx(ArticleSurface, { mode: articleSurfaceMode, html: articleEditorHtml, styleCss: previewStyleQuery.data?.css ?? '', title: `Article preview ${detailPanel.familyTitle}`, onChange: setArticleEditorHtml, savedHtml: savedArticleHtml, onSave: () => void handleSaveArticleEdit(), onRestore: handleRestoreArticleEdit, saving: savingArticleEdit, error: articleEditorError }), _jsx(PlaceholderBlocks, { placeholders: detailPanel.detail.placeholders })] })) : (_jsx(EmptyState, { title: "No preview", description: "No preview HTML available for this article." }))), detailPanel.activeTab === 'source' && (detailPanel.detail.sourceHtml ? (_jsx(EditorPane, { className: "detail-source-editor-pane", children: _jsx(CodeEditor, { value: detailPanel.detail.sourceHtml, language: "html", readOnly: true }) })) : (_jsx(EmptyState, { title: "No source", description: "No source HTML available." }))), detailPanel.activeTab === 'history' && (_jsx(HistoryTimeline, { revisions: detailPanel.revisions })), detailPanel.activeTab === 'lineage' && (_jsx(LineagePanel, { entries: detailPanel.detail.lineage })), detailPanel.activeTab === 'publish' && (_jsx(PublishLogPanel, { records: detailPanel.detail.publishLog })), detailPanel.activeTab === 'pbis' && (_jsx(PBIPanel, { pbis: detailPanel.detail.relatedPbis })), detailPanel.activeTab === 'relations' && activeWorkspace && (_jsx(RelationsPanel, { workspaceId: activeWorkspace.id, familyId: detailPanel.detail.familyId, relations: detailPanel.detail.relations, onChanged: reloadCurrentDetail, onOpenRelation: openRelatedFamily }))] }));
    };
    /* ---------- No workspace state ---------- */
    if (!activeWorkspace) {
    ...
    return (_jsxs(_Fragment, { children: [_jsx(PageHeader, { title: "Articles", subtitle: `${filterCounts.all} article families`, actions: _jsxs("div", { className: "explorer-search-wrapper", children: [_jsx(IconSearch, { size: 13, className: "explorer-search-icon" }), _jsx("input", { className: "input input-sm", placeholder: "Search articles...", style: { width: 240 }, value: searchText, onChange: (e) => setSearchText(e.target.value) })] }) }), _jsx("div", { className: "route-content", children: _jsxs("div", { className: "explorer-layout", children: [_jsxs("div", { className: "explorer-filter-rail", children: [_jsx("div", { className: "explorer-filter-heading", children: "Filter" }), _jsx("div", { className: "explorer-filter-list", children: filters.map((f) => (_jsxs("button", { className: `explorer-filter-btn${activeFilter === f.id ? ' active' : ''}`, onClick: () => setActiveFilter(f.id), children: [_jsx("span", { children: f.label }), _jsx("span", { className: "explorer-filter-count", children: f.count })] }, f.id))) }), availableLocales.length > 0 && (_jsxs(_Fragment, { children: [_jsx("div", { className: "divider" }), _jsx("div", { className: "explorer-filter-heading", children: "Locale" }), _jsxs("div", { className: "explorer-filter-list", children: [_jsxs("button", { className: `explorer-filter-btn${!selectedLocale ? ' active' : ''}`, onClick: () => setSelectedLocale(null), children: [_jsx(IconGlobe, { size: 12 }), _jsx("span", { children: "All locales" })] }), availableLocales.map((loc) => (_jsx("button", { className: `explorer-filter-btn${selectedLocale === loc ? ' active' : ''}`, onClick: () => setSelectedLocale(loc), children: loc }, loc)))] })] }))] }), _jsxs("div", { className: "explorer-main", children: [latestSuccessfulSync && latestSuccessfulSync.endedAtUtc && (() => {
                                ...
                                /* Folder tree view */
                                _jsx(_Fragment, { children: _jsxs("div", { className: "explorer-directory-shell", children: [_jsxs("div", { className: "explorer-directory-header", children: [_jsxs("div", { children: [_jsx("div", { className: "explorer-directory-eyebrow", children: "Directory" }), _jsx("div", { className: "explorer-directory-title", children: "Article library" })] }), _jsxs("div", { className: "explorer-directory-summary", children: [_jsxs(Badge, { variant: "neutral", children: [filteredTree.length, " famil", filteredTree.length === 1 ? 'y' : 'ies'] }), visibleFolderCount > 0 && (_jsxs(Badge, { variant: "neutral", children: [visibleFolderCount, " folder", visibleFolderCount !== 1 ? 's' : ''] }))] })] }), folderTree.some((item) => item.type === 'folder') && (_jsxs("div", { className: "explorer-tree-toolbar", children: [_jsx("div", { className: "explorer-tree-toolbar-copy", children: "Collapse and expand state is saved per workspace." }), _jsxs("div", { className: "explorer-tree-toolbar-actions", children: [_jsx("button", { className: "btn btn-ghost btn-xs", onClick: expandAllFolders, children: "Expand all" }), _jsx("button", { className: "btn btn-ghost btn-xs", onClick: collapseAllFolders, children: "Collapse all" })] })] })), _jsx(FolderTreeView, { items: folderTree, expandedFolders: expandedFolders, onToggleFolder: toggleFolder, detailPanel: detailPanel, openArticleDetail: openArticleDetail })] }) }))] })] }) }), _jsx(Drawer, { open: detailPanel.open, onClose: () => setDetailPanel((state) => ({ ...state, open: false })), title: detailPanel.familyTitle, variant: "fullscreen", customHeader: (() => {
                    const selectedLocaleInfo = detailPanel.localeVariants.find((v) => v.localeVariantId === detailPanel.localeVariantId);
                    return (_jsxs("div", { className: "article-detail-toolbar", children: [_jsxs("div", { className: "article-detail-toolbar-top", children: [_jsx("button", { className: "article-detail-back", onClick: () => setDetailPanel((s) => ({ ...s, open: false })), "aria-label": "Back to articles", children: _jsx(IconChevronLeft, { size: 16 }) }), _jsxs("div", { className: "article-detail-title-group", children: [_jsx("h2", { className: "article-detail-title", children: detailPanel.familyTitle }), _jsxs("div", { className: "article-detail-badges", children: [_jsx(StatusChip, { status: selectedLocaleInfo ? revisionStateToBadge(selectedLocaleInfo.revision.state) : 'live' }), selectedLocaleInfo && selectedLocaleInfo.revision.draftCount > 0 && (_jsxs(Badge, { variant: "primary", children: [selectedLocaleInfo.revision.draftCount, " draft", selectedLocaleInfo.revision.draftCount !== 1 ? 's' : ''] })), selectedLocaleInfo?.hasConflicts && (_jsx(Badge, { variant: "danger", children: "Conflict" }))] })] }), _jsxs("div", { className: "article-detail-toolbar-end", children: [detailPanel.activeTab === 'preview' && (_jsx(ArticleModeToggle, { mode: articleSurfaceMode, onChange: setArticleSurfaceMode, compact: true })), articleSaveMessage && detailPanel.activeTab === 'preview' && (_jsx("span", { className: "article-surface-header__note", children: articleSaveMessage })), detailPanel.localeVariants.length > 1 && (_jsx("select", { className: "article-detail-locale-select", value: detailPanel.localeVariantId, onChange: async (event) => {
                                                ...
                                                }, children: detailPanel.localeVariants.map((locale) => (_jsxs("option", { value: locale.localeVariantId, children: [locale.locale, locale.revision.draftCount > 0 ? ` (${locale.revision.draftCount} drafts)` : ''] }, locale.localeVariantId))) })), selectedLocaleInfo?.revision.updatedAtUtc && (() => {
                                                const info = formatSyncAge(selectedLocaleInfo.revision.updatedAtUtc);
                                                return (_jsx("span", { className: `sync-freshness-badge sync-freshness-badge--${info.freshness}`, children: info.label }));
                                            })()] })] }), _jsx("div", { className: "article-detail-tab-bar", role: "tablist", children: DETAIL_TAB_CONFIG.map((tab) => (_jsxs("button", { role: "tab", "aria-selected": detailPanel.activeTab === tab.id, className: `article-detail-tab${detailPanel.activeTab === tab.id ? ' active' : ''}`, onClick: () => setDetailPanel((current) => ({ ...current, activeTab: tab.id })), children: [_jsx(tab.icon, { size: 12 }), tab.label] }, tab.id))) })] }));
