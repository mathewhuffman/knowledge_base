...
export function AnalysisJobRunner({ workspaceId, batchId, startOnOpen, onComplete }) {
    ...
    useEffect(() => {
        ...
        const handler = (event) => {
            ...
            const eventModel = event?.metadata?.agentModelId;
            if (typeof eventModel === 'string' && eventModel.trim()) {
                setCurrentRunModel(eventModel.trim());
            }
            const orchestration = parseRuntimeStatusFromUnknown(event?.metadata?.orchestration);
            if (orchestration) {
