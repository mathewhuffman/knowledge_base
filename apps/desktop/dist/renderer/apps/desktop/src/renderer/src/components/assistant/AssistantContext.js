...
export function AiAssistantProvider({ windowRole, activeRoute, workspaceId, children }) {
    const [presentation, setPresentation] = useState(DEFAULT_PRESENTATION_STATE);
    ...
    const fallbackContext = useMemo(() => {
        if (windowRole !== 'main' || !workspaceId || !activeRoute)
            return null;
        return {
            workspaceId,
            route: activeRoute,
    ...
    const open = useMemo(() => (windowRole === 'main'
        ? presentation.state === 'embedded_open'
        : presentation.state === 'detached_panel'), [presentation.state, windowRole]);
    const hasUnread = presentation.hasUnread;
    useEffect(() => {
        if (!window.kbv?.invoke) {
            return;
        }
        void window.kbv.invoke('ai.assistant.presentation.get')
            .then((response) => {
        ...
        void window.kbv.invoke('ai.assistant.context.current')
            .then((response) => {
    ...
    const transitionPresentation = useCallback(async (transition) => {
        if (!window.kbv?.invoke) {
            throw new Error('Assistant IPC bridge is unavailable.');
        }
        const response = await window.kbv.invoke('ai.assistant.presentation.transition', {
            transition
        });
        if (!response.ok || !response.data?.state) {
    ...
    const setOpen = useCallback((nextOpen) => {
        const transition = windowRole === 'main'
            ? { type: nextOpen ? 'open_embedded_panel' : 'close_embedded_panel' }
            : { type: nextOpen ? 'open_detached_panel' : 'collapse_detached_to_launcher' };
        void transitionPresentation(transition).catch((transitionError) => {
            setError((current) => current ?? String(transitionError));
        });
    }, [transitionPresentation, windowRole]);
    const setEmbeddedLauncherPosition = useCallback(async (position) => {
        await transitionPresentation({
            type: 'set_embedded_launcher_position',
            position
        });
    }, [transitionPresentation]);
    const detachLauncher = useCallback(async (anchorPoint) => {
        await transitionPresentation({
            type: 'detach_launcher',
            anchorPoint
        });
    }, [transitionPresentation]);
    const detachPanel = useCallback(async (anchorPoint) => {
        await transitionPresentation({
            type: 'detach_panel',
            anchorPoint
        });
