...
import { buildTimelineEntriesWithSkippedStages, deriveCompletedStages, deriveSkippedStages, getVisibleStage, getVisibleStageLabel, STAGE_LABELS, ROLE_LABELS, } from './helpers';
...
export function BatchAnalysisInspector({ inspection, runtimeStatus, eventStream, isRunning, onOpenSession, onRefresh, }) {
    ...
    const skippedStages = useMemo(() => deriveSkippedStages(inspection.timeline, stageEvents ?? []), [inspection.timeline, stageEvents]);
    const renderableTimelineCount = useMemo(() => buildTimelineEntriesWithSkippedStages(inspection.timeline, stageEvents ?? []).length, [inspection.timeline, stageEvents]);
