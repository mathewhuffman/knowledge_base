...
export function buildPreviewDiffHtml(beforeHtml, afterHtml) {
...
function applyNodeDiff(beforeParent, afterParent, targetDoc) {
    const beforeChildren = getComparableChildren(beforeParent);
    const afterChildren = getComparableChildren(afterParent);
    const sequence = diffSequence(beforeChildren.map((node) => getNodeIdentity(node)), afterChildren.map((node) => getNodeIdentity(node)));
...
function getComparableChildren(node) {
...
function shouldDiffChildren(beforeNode, afterNode) {
    if (beforeNode.nodeType !== Node.ELEMENT_NODE || afterNode.nodeType !== Node.ELEMENT_NODE) {
        return false;
    }
    const beforeElement = beforeNode;
    const afterElement = afterNode;
...
function hasComparableChildElements(element) {
...
function markAddedNode(node, targetDoc) {
    if (node.nodeType === Node.ELEMENT_NODE) {
        node.classList.add(DIFF_ADDED_CLASS);
        return node;
    }
...
function createRemovedNode(node, targetDoc) {
    if (node.nodeType === Node.ELEMENT_NODE) {
