...
class BatchAnalysisOrchestrator {
    ...
    buildWorkerPrompt(plan, extraInstructions) {
        return [
    ...
    findLikelyMissingCreateTargets(plan, plannerPrefetch) {
        ...
        const representedCreatePbiIds = new Set(plan.items
            .filter((item) => item.action === 'create')
            .flatMap((item) => item.pbiIds.map((pbiId) => pbiId.trim()))
            .filter(Boolean));
        const clusterMatchesById = new Map();
        ...
        const likelyMissingCreates = (plannerPrefetch.topicClusters ?? [])
            ...
            const hasStrongExistingSignal = clusterMatches.some((match) => (match.topResults ?? []).some((candidate) => {
...
function collectDeterministicExistingArticleSignals(plannerPrefetch) {
    if (!plannerPrefetch) {
        return [];
    }
    ...
    const signals = [];
    const seen = new Set();
    const pushSignal = (title, familyId) => {
