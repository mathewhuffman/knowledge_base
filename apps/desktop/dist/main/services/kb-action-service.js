...
exports.KbActionService = void 0;
...
function normalizeProposalAction(action) {
...
function normalizeRelatedPbiIds(values) {
    if (!values?.length) {
        return undefined;
    }
    const normalized = Array.from(new Set(values.map((value) => value.trim()).filter(Boolean))).sort((left, right) => left.localeCompare(right));
...
function buildCreateProposalIdempotencyKey(params) {
    const normalized = {
        ...
        proposal: {
            ...
            metadata: params.proposal.metadata && typeof params.proposal.metadata === 'object' && !Array.isArray(params.proposal.metadata)
                ? params.proposal.metadata
                : {}
        }
    };
    return (0, node_crypto_1.createHash)('sha256').update(stableStringify(normalized)).digest('hex');
}
class KbActionService {
    deps;
    constructor(deps) {
        this.deps = deps;
    }
    ...
    async listArticleTemplates(input) {
    ...
    async createProposals(input) {
        const proposals = input.proposals ?? [];
        if (proposals.length === 0) {
            throw new Error('create_proposals requires at least one proposal item');
        }
        const created = await Promise.all(proposals.map(async (proposal) => {
...
exports.KbActionService = KbActionService;
