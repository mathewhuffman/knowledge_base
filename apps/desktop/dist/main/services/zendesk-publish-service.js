...
function normalizeHtmlForConflict(value) {
    return (value ?? '')
        .replace(/\r\n/g, '\n')
        .replace(/\s+/g, ' ')
        .trim();
...
function issueHasError(issue) {
    return issue.severity === shared_types_1.DraftValidationSeverity.ERROR;
}
function itemHasWarnings(item) {
    return item.issues.some((issue) => issue.severity === shared_types_1.DraftValidationSeverity.WARNING);
}
function canPublishItem(item, publishTarget, settings) {
    if (!item.canPublish) {
        return false;
    }
...
function buildItemStatus(item, publishTarget, settings) {
    const hasConflict = item.issues.some((issue) => issue.code === shared_types_1.ZendeskPublishValidationCode.REMOTE_CONFLICT);
    if (canPublishItem(item, publishTarget, settings)) {
        return shared_types_1.PublishJobItemState.QUEUED;
    }
    return hasConflict ? shared_types_1.PublishJobItemState.CONFLICTED : shared_types_1.PublishJobItemState.BLOCKED;
}
function summarizeValidation(items) {
    return items.reduce((summary, item) => {
        ...
        if (item.issues.some((issue) => issue.code === shared_types_1.ZendeskPublishValidationCode.REMOTE_CONFLICT)) {
            summary.conflicts += 1;
        }
        if (item.issues.some((issue) => issue.severity === shared_types_1.DraftValidationSeverity.WARNING)) {
            summary.warnings += 1;
        }
        return summary;
    }, {
...
function mapDraftValidationIssue(warning, settings) {
    if (warning.code === shared_types_1.DraftValidationCode.UNRESOLVED_PLACEHOLDER) {
        if (settings.zendeskPlaceholderAssetPolicy === 'upload') {
            return {
                code: shared_types_1.ZendeskPublishValidationCode.DRAFT_VALIDATION,
                severity: shared_types_1.DraftValidationSeverity.INFO,
        ...
        return {
            code: shared_types_1.ZendeskPublishValidationCode.PLACEHOLDER_BLOCKED,
    ...
    if (warning.code === shared_types_1.DraftValidationCode.MISSING_PLACEMENT) {
        return null;
    }
    if (warning.code === shared_types_1.DraftValidationCode.LOCALE_ISSUE) {
        return {
            code: shared_types_1.ZendeskPublishValidationCode.LOCALE_DISABLED,
            severity: shared_types_1.DraftValidationSeverity.ERROR,
...
function placeholderFileName(description, locale, index) {
...
function buildPlaceholderSvg(description) {
    const lines = wrapPlaceholderText(description.trim() || 'Placeholder image');
    const lineHeight = 28;
    const width = 1280;
    const height = 720;
    const firstY = 300 - ((lines.length - 1) * lineHeight) / 2;
    const text = lines.map((line, index) => (`<text x="640" y="${firstY + (index * lineHeight)}" text-anchor="middle" font-size="28" font-family="Arial, sans-serif" fill="#0f172a">${escapeHtml(line)}</text>`)).join('');
    return [
...
function wrapPlaceholderText(value) {
...
class ZendeskPublishService {
    ...
    async validate(input) {
        ...
        try {
            ...
            const locales = await client.listEnabledLocales().catch((error) => {
                logger_1.logger.warn('zendesk-publish-service locale fetch failed', {
                    workspaceId,
                    message: error instanceof Error ? error.message : String(error)
                });
                return [];
            });
            context.remoteLocales = new Set(locales
        ...
        for (const branchId of branchIds) {
            ...
            if (branchDetail.branch.status !== shared_types_1.DraftBranchStatus.READY_TO_PUBLISH) {
                issues.push({
                    code: shared_types_1.ZendeskPublishValidationCode.BRANCH_NOT_READY,
                    severity: shared_types_1.DraftValidationSeverity.ERROR,
            ...
            if (!branchDetail.branch.liveRevisionId && !placementSectionId && !placementSectionName) {
                if (!settings.zendeskAllowSectionCreation) {
                    issues.push({
                        code: shared_types_1.ZendeskPublishValidationCode.MISSING_PLACEMENT,
                        severity: shared_types_1.DraftValidationSeverity.ERROR,
            ...
            if (context.clientError) {
                issues.push({
                    code: shared_types_1.ZendeskPublishValidationCode.ZENDESK_CONFIGURATION,
                    severity: shared_types_1.DraftValidationSeverity.ERROR,
                ...
                issues.push({
                    code: shared_types_1.ZendeskPublishValidationCode.REMOTE_LOCALE_DISABLED,
                    severity: shared_types_1.DraftValidationSeverity.ERROR,
            ...
            if (articleId && !context.clientError) {
                try {
                    const remoteArticle = await context.client.showArticle(articleId, branchDetail.branch.locale).catch(async (error) => {
                        if (error instanceof zendesk_client_1.ZendeskApiError && error.status === 404 && !branchDetail.branch.liveRevisionId) {
                    ...
                    if (branchDetail.branch.liveRevisionId) {
                        ...
                        if (localLiveHtml !== remoteHtml || (localSectionId && remoteSectionId && localSectionId !== remoteSectionId)) {
                            issues.push({
                                code: shared_types_1.ZendeskPublishValidationCode.REMOTE_CONFLICT,
                                severity: shared_types_1.DraftValidationSeverity.ERROR,
                    ...
                    if (error instanceof zendesk_client_1.ZendeskApiError && error.status === 404) {
                        issues.push({
                            code: shared_types_1.ZendeskPublishValidationCode.REMOTE_ARTICLE_MISSING,
                            severity: shared_types_1.DraftValidationSeverity.ERROR,
    ...
    async runPublish(input, emit, command, jobId, isCancelled) {
        const workspaceId = input.workspaceId?.trim();
        if (!workspaceId) {
            emit({
                id: jobId,
        ...
        const publishTarget = input.publishTarget === 'draft' ? 'draft' : 'live';
        const settings = await this.workspaceRepository.getWorkspaceSettings(workspaceId);
        ...
        await this.workspaceRepository.updatePublishJobStatus(workspaceId, jobId, shared_types_1.PublishStatus.RUNNING, {
            startedAtUtc
        });
        await this.workspaceRepository.replacePublishJobItems(workspaceId, jobId, validation.items.map((item) => ({
        ...
        if (publishable.length === 0) {
            ...
            await this.workspaceRepository.updatePublishJobStatus(workspaceId, jobId, shared_types_1.PublishStatus.FAILED, {
                startedAtUtc,
                completedAtUtc
            });
            emit({
        ...
        try {
            for (const [index, item] of publishable.entries()) {
                ...
                await this.workspaceRepository.updatePublishJobItem(workspaceId, jobId, item.branchId, {
                    status: shared_types_1.PublishJobItemState.RUNNING,
                ...
                try {
                    ...
                    await this.workspaceRepository.updatePublishJobItem(workspaceId, jobId, item.branchId, {
                        status: shared_types_1.PublishJobItemState.SUCCEEDED,
                        zendeskArticleId: result.zendeskArticleId,
                        zendeskSourceArticleId: result.zendeskSourceArticleId,
                    ...
                    if (conflict) {
                        await this.workspaceRepository.setDraftBranchStatus({
                            workspaceId,
                            branchId: item.branchId,
                            status: shared_types_1.DraftBranchStatus.CONFLICTED
                        });
                    }
                    await this.workspaceRepository.updatePublishJobItem(workspaceId, jobId, item.branchId, {
                        status: conflict ? shared_types_1.PublishJobItemState.CONFLICTED : shared_types_1.PublishJobItemState.FAILED,
                        resultCode: conflict ? shared_types_1.PublishJobItemState.CONFLICTED : shared_types_1.PublishJobItemState.FAILED,
                        resultMessage: message,
                        completedAtUtc: new Date().toISOString()
                    });
                    logger_1.logger.warn('zendesk-publish-service item publish failed', {
                        workspaceId,
                        branchId: item.branchId,
                        message
                    });
            ...
            if (error instanceof Error && error.message === 'publish_cancelled') {
                ...
                await this.workspaceRepository.updatePublishJobStatus(workspaceId, jobId, shared_types_1.PublishStatus.CANCELED, {
                    startedAtUtc,
        ...
        await this.workspaceRepository.updatePublishJobStatus(workspaceId, jobId, allSucceeded ? shared_types_1.PublishStatus.COMPLETED : shared_types_1.PublishStatus.FAILED, {
            startedAtUtc,
            completedAtUtc: endedAtUtc
        });
        emit({
    ...
    async resolveBranchIds(workspaceId, branchIds) {
        ...
        return branches.branches
            .filter((branch) => branch.status === shared_types_1.DraftBranchStatus.READY_TO_PUBLISH)
            .map((branch) => branch.id);
    }
    async publishItem(client, item, execution) {
        ...
        if (articleId) {
            ...
            const hasPlacementIntent = Boolean(family.sectionId
                || family.sectionName
        ...
        const createdArticle = await client.createArticleInSectionWithOptions(resolvedSection.sectionId, createPayload, { notifySubscribers: execution.publishTarget === 'live' ? execution.settings.zendeskNotifySubscribers : false });
        return {
    ...
    async materializePublishHtml(client, html, item, settings) {
    ...
    async uploadInlineDataImages(client, html, item) {
        const pattern = /<img\b([^>]*?)src=(["'])(data:image\/[^"']+)\2([^>]*)>/gi;
        const matches = Array.from(html.matchAll(pattern));
        let nextHtml = html;
        for (const [index, match] of matches.entries()) {
            const parsed = this.parseDataImageUri(match[3]);
            const media = await this.uploadGuideMediaBinary(client, parsed.contentType, parsed.bytes, `inline-image-${item.locale}-${index + 1}.${parsed.extension}`);
            nextHtml = nextHtml.replace(match[3], media.url);
        }
        return nextHtml;
    }
    parseDataImageUri(dataUri) {
    ...
    async uploadPlaceholderGuideMedia(client, item, description, index) {
        const svg = buildPlaceholderSvg(description);
        return this.uploadGuideMediaBinary(client, 'image/svg+xml', Buffer.from(svg, 'utf8'), placeholderFileName(description, item.locale, index));
    }
    async uploadGuideMediaBinary(client, contentType, bytes, filename) {
    ...
    async resolveTargetSection(client, item, family, settings) {
        const explicitSectionId = parsePositiveInteger(item.placement?.sectionId ?? family.sectionId ?? family.sourceSectionId);
    ...
    async findOrCreateCategory(client, item, family, settings) {
        const explicitCategoryId = parsePositiveInteger(item.placement?.categoryId ?? family.categoryId ?? family.sourceCategoryId);
    ...
    async findOrCreateSection(client, categoryId, locale, sectionName, settings) {
    ...
    async cancelQueuedItems(workspaceId, jobId) {
        const snapshot = await this.workspaceRepository.getPublishJobSnapshot(workspaceId, jobId);
        const updates = snapshot.items
            .filter((item) => item.status === shared_types_1.PublishJobItemState.QUEUED || item.status === shared_types_1.PublishJobItemState.RUNNING)
            .map((item) => this.workspaceRepository.updatePublishJobItem(workspaceId, jobId, item.branchId, {
            status: shared_types_1.PublishJobItemState.CANCELED,
            resultCode: shared_types_1.PublishJobItemState.CANCELED,
...
exports.ZendeskPublishService = ZendeskPublishService;
