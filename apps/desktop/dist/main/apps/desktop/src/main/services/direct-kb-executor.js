...
exports.DirectKbExecutor = void 0;
const shared_types_1 = require("@kb-vault/shared-types");
const DIRECT_STAGE_ACTIONS = {
    planner: new Set(shared_types_1.DIRECT_BATCH_READ_ONLY_ACTION_TYPES),
    'plan-reviewer': new Set(shared_types_1.DIRECT_BATCH_READ_ONLY_ACTION_TYPES),
    worker: new Set(shared_types_1.DIRECT_BATCH_WORKER_ACTION_TYPES),
    'final-reviewer': new Set(shared_types_1.DIRECT_BATCH_READ_ONLY_ACTION_TYPES)
};
const DIRECT_ARTICLE_EDIT_ACTIONS = new Set(shared_types_1.DIRECT_ARTICLE_EDIT_ACTION_TYPES);
const DIRECT_ASSISTANT_READ_ACTIONS = new Set(shared_types_1.DIRECT_ASSISTANT_READ_ACTION_TYPES);
const DIRECT_ASSISTANT_TEMPLATE_ACTIONS = new Set(shared_types_1.DIRECT_ASSISTANT_TEMPLATE_ACTION_TYPES);
class DirectKbExecutor {
    deps;
    constructor(deps) {
        this.deps = deps;
    }
    ...
    assertActionAllowed(action, requestContext) {
        const allowed = this.resolveAllowedActions(requestContext);
        if (!allowed?.has(action.type)) {
            const descriptor = requestContext.sessionType === 'assistant_chat'
        ...
        switch (request.action.type) {
            ...
            case 'create_proposals': {
                ...
                for (const proposal of proposals) {
                    ...
                    if ((action === shared_types_1.ProposalAction.EDIT || action === shared_types_1.ProposalAction.RETIRE)
            ...
            case 'patch_form': {
                ...
                if (!directContext?.allowPatchForm
                    || !directContext.route
            ...
            default:
                throw new Error(`Unsupported direct action ${request.action.type ?? 'unknown'}`);
        }
    }
    requireBatchId(requestedBatchId, scopedBatchId) {
    ...
    mergeScopedIds(requested, allowed, label) {
        if (!allowed?.length) {
            return requested;
        }
        if (!requested?.length) {
    ...
    assertLocaleVariantAllowed(localeVariantId, allowedLocaleVariantIds) {
    ...
    assertFamilyAllowed(familyId, allowedFamilyIds) {
    ...
    normalizeProposalMutationAction(action) {
        if (action === shared_types_1.ProposalAction.CREATE || action === shared_types_1.ProposalAction.EDIT || action === shared_types_1.ProposalAction.RETIRE) {
            return action;
        }
        throw new Error(`Unsupported proposal mutation action ${action}`);
    }
}
exports.DirectKbExecutor = DirectKbExecutor;
