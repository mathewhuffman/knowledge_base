...
import type {
  ...
  BatchPlanItem,
  PBIBatchAnalysisConfig,
  PBIBatchGuaranteedCreateConflict,
...
interface BatchPlanExecutionValidation {
  finalQuestions: BatchAnalysisQuestion[];
  ok: boolean;
  ...
  missingGuaranteedEditTargets: string[];
  missingGuaranteedCreateTargets: string[];
}

interface QuestionMergeMeta {
  explicitAnswer?: boolean;
  explicitAnsweredAtUtc?: boolean;
  explicitId?: boolean;
  explicitPrompt?: boolean;
  explicitQuestionSetId?: boolean;
  explicitReason?: boolean;
  explicitRequiresUserInput?: boolean;
  explicitStatus?: boolean;
}

type MergeableBatchAnalysisQuestion = BatchAnalysisQuestion & {
  __mergeMeta?: QuestionMergeMeta;
};

function buildProposalTemplateGuidanceSection(
  template: TemplatePackDetail | null | undefined,
...
export class BatchAnalysisOrchestrator {
  ...
  private buildExecutedItems(
    ...
    return planItems.map((item) => {
      ...
      const currentSessionMatches = remainingProposals.filter((proposal) =>
      ...
      const currentSessionExactMatch = currentSessionMatches.find((proposal) =>
        doesPlacementMeetRequirement(proposal.suggestedPlacement, item.suggestedPlacement)
      );
      ...
      const fallbackMatches = remainingProposals.filter((proposal) =>
        proposal.action === item.action
        && this.normalizeTitle(proposal.targetTitle ?? '') === normalizedTargetTitle
      );
      const fallbackExactMatch = fallbackMatches.find((proposal) =>
        doesPlacementMeetRequirement(proposal.suggestedPlacement, item.suggestedPlacement)
      );
  ...
  buildPlannerPrompt(params: {
    ...
    proposalTemplate?: TemplatePackDetail | null;
  }): string {
    ...
    return [
      ...
      buildProposalTemplateGuidanceSection(
        params.proposalTemplate,
        'Use this active template when reasoning about the expected structure, tone, and placeholder conventions of any proposal HTML the worker will later draft.'
      ),
  ...
  buildPlanReviewerPrompt(params: {
    ...
    plan: BatchAnalysisPlan;
    plannerPrefetch?: BatchPlannerPrefetch;
    candidateQuestions?: BatchAnalysisQuestion[];
    proposalTemplate?: TemplatePackDetail | null;
  }): string {
    const hasUserDirectives = batchContextHasUserDirectives(params.batchContext);
    return [
      ...
      buildProposalTemplateGuidanceSection(
        params.proposalTemplate,
        'Use this active template as the review baseline for proposal structure, tone, and placeholder conventions whenever you judge whether the plan is likely to produce acceptable proposal HTML.'
      ),
      'Batch context summary:',
      JSON.stringify(compactBatchContextForPrompt(params.batchContext), null, 2),
      'Uploaded PBI summary:',
  ...
  buildReviewCandidateQuestions(params: {
    plan: BatchAnalysisPlan;
    plannerPrefetch?: BatchPlannerPrefetch;
    existingQuestions?: BatchAnalysisQuestion[];
    discoveredWork?: BatchDiscoveredWorkItem[];
    batchContext?: unknown;
  }): BatchAnalysisQuestion[] {
    const baseQuestions = mergeStructuredQuestions(
      params.existingQuestions ?? [],
      params.plan.questions ?? []
    );
  ...
  private synthesizeDeterministicReviewCandidateQuestions(params: {
    plan: BatchAnalysisPlan;
    plannerPrefetch?: BatchPlannerPrefetch;
    discoveredWork?: BatchDiscoveredWorkItem[];
  }): BatchAnalysisQuestion[] {
    ...
    return mergeStructuredQuestions(
      assessments.flatMap((assessment): BatchAnalysisQuestion[] => {
        ...
        const deterministicQuestions: BatchAnalysisQuestion[] = [];
        ...
        if (shouldSynthesizeStandaloneVsFoldQuestion(assessment)) {
          ...
          if (existingArticleTitle) {
            deterministicQuestions.push({
              id: createDeterministicQuestionId(
                'standalone-vs-fold',
                assessment.clusterId,
                `${assessment.displayTitle}:${existingArticleTitle}`
              ),
  ...
  private synthesizeUserDirectiveReviewCandidateQuestions(params: {
    plan: BatchAnalysisPlan;
    batchContext?: unknown;
  }): BatchAnalysisQuestion[] {
  ...
  private normalizeStructuredQuestions(params: {
    ...
    const structuredQuestions = Array.isArray(params.rawQuestions)
      ? params.rawQuestions.flatMap((question): BatchAnalysisQuestion[] => {
          ...
          const normalizedQuestion: MergeableBatchAnalysisQuestion = {
  ...
  validatePlanForExecution(params: {
    ...
    let finalQuestions = mergeStructuredQuestions(
      params.candidateQuestions ?? params.plan.questions ?? [],
      params.review.questions ?? []
    );
  ...
  applyDeterministicPlanReviewGuard(params: {
    ...
    const malformedNeedsUserInput =
      params.review.verdict === 'needs_user_input'
    ...
    const forcedRevision =
      params.review.verdict === 'approved'
  ...
  async buildWorkerPrompt(
    plan: BatchAnalysisPlan,
    extraInstructionsOrOptions?: string | {
    ...
    return [
      ...
      buildProposalTemplateGuidanceSection(
        proposalTemplate,
        'When you persist `proposedHtml`, treat this active template as authoritative guidance for structure, tone, examples, and placeholder conventions.'
      ),
  ...
  buildFinalReviewerPrompt(params: {
    ...
    return [
      ...
      buildProposalTemplateGuidanceSection(
        params.proposalTemplate,
        'Use this active template when judging whether the executed proposals match the expected proposal HTML structure, tone, and placeholder conventions.'
      ),
      'Batch context summary:',
      JSON.stringify(compactBatchContextForPrompt(params.batchContext), null, 2),
      'Uploaded PBI summary:',
  ...
  private validateUserDirectedTargets(params: {
    plan: BatchAnalysisPlan;
    finalQuestions: BatchAnalysisQuestion[];
    batchContext?: unknown;
  }): {
    blockingQuestions: BatchAnalysisQuestion[];
    missingGuaranteedEditTargets: string[];
    missingGuaranteedCreateTargets: string[];
  } {
    ...
    const blockingQuestions: BatchAnalysisQuestion[] = [];
    const missingGuaranteedEditTargets: string[] = [];
    const missingGuaranteedCreateTargets: string[] = [];

    for (const family of analysisConfig.guaranteedEditFamilies) {
      for (const variant of family.resolvedLocaleVariants) {
        const hasMatchingEdit = params.plan.items.some((item) =>
          item.action === 'edit'
          && (
        ...
        if (!hasMatchingEdit) {
          missingGuaranteedEditTargets.push(
            `Guaranteed edit target ${family.familyTitle} (${variant.locale}) is missing a concrete edit plan item.`
          );
        }
      }
    }
    ...
    for (const article of analysisConfig.guaranteedCreateArticles) {
      ...
      if (matchingConflict) {
        const questionId = createDeterministicQuestionId(
          'guaranteed-create-conflict',
          matchingConflict.clientId,
          matchingConflict.title
        );
      ...
      const matchingCreate = params.plan.items.find((item) =>
        item.action === 'create'
      ...
      if (!matchingCreate) {
        missingGuaranteedCreateTargets.push(
          `Guaranteed create target ${article.title} (${article.targetLocale}) is missing a create plan item with the correct locale.`
        );
        continue;
      }
      ...
      if (!doesPlacementMeetRequirement(matchingCreate.suggestedPlacement, article)) {
        missingGuaranteedCreateTargets.push(
          `Guaranteed create target ${article.title} (${article.targetLocale}) is missing the requested category or section placement in its create plan item.`
        );
      }
    }
  ...
  validateFinalApproval(params: {
    ...
    if (directiveValidation.blockingQuestions.length > 0) {
      reasons.push(
        `Final approval blocked by ${directiveValidation.blockingQuestions.length} unresolved guaranteed-create ambiguity question(s).`
      );
    }
...
function batchContextHasUserDirectives(batchContext: unknown): boolean {
...
function compactBatchUserDirectivesForPrompt(batchContext: Record<string, unknown>): Record<string, unknown> | null {
  const { analysisConfig, guaranteedCreateConflicts } = extractBatchUserDirectives(batchContext);
  if (!analysisConfig) {
    return null;
  }
...
function extractBatchUserDirectives(batchContext: unknown): {
  analysisConfig: PBIBatchAnalysisConfig | null;
  ...
  const analysisConfig =
    ...
    && Array.isArray((analysisConfigCandidate as PBIBatchAnalysisConfig).guaranteedEditFamilies)
    && Array.isArray((analysisConfigCandidate as PBIBatchAnalysisConfig).guaranteedCreateArticles)
      ? analysisConfigCandidate as PBIBatchAnalysisConfig
      : null;
  const guaranteedCreateConflicts = Array.isArray(record.guaranteedCreateConflicts)
    ? record.guaranteedCreateConflicts.filter(
        (conflict): conflict is PBIBatchGuaranteedCreateConflict =>
          Boolean(conflict && typeof conflict === 'object' && typeof (conflict as PBIBatchGuaranteedCreateConflict).title === 'string')
      )
    : [];
...
function compactPlacementForPrompt(
...
function mergePlanReviewDelta(
  existing: BatchPlanReviewDelta | undefined,
  incoming: BatchPlanReviewDelta
): BatchPlanReviewDelta {
  const baseline: BatchPlanReviewDelta = existing ?? {
    summary: '',
    requestedChanges: [],
    missingPbiIds: [],
    missingCreates: [],
...
function shouldSynthesizeIncludeOrDeferQuestion(assessment: DeterministicClusterCoverageAssessment): boolean {
  const hasOnlyNonDecisiveCoverage =
    assessment.relatedPlanItems.length === 0
    || assessment.relatedPlanItems.every((item) => item.action === 'no_impact');
  const hasConcreteCoverageSignal =
...
function shouldSynthesizeStandaloneVsFoldQuestion(assessment: DeterministicClusterCoverageAssessment): boolean {
...
function createDeterministicQuestionId(kind: string, clusterId: string, title: string): string {
...
function normalizeLocaleForComparison(value?: string | null): string {
  return value?.trim().toLowerCase() ?? '';
}

function normalizePlacementScopeId(value?: string | null): string {
  return value?.trim() ?? '';
}
...
function normalizeBatchPlanPlacement(value: unknown): ProposalPlacementSuggestion | undefined {
...
interface PlannerTaxonomySectionEntry {
  sectionId: string;
  sectionName: string;
  categoryId: string;
  categoryName: string;
...
interface PlannerTaxonomyIndex {
  categoriesById: Map<string, { categoryName: string }>;
  categoryIdsByName: Map<string, string[]>;
  sectionsById: Map<string, PlannerTaxonomySectionEntry>;
  sectionsByCategoryId: Map<string, PlannerTaxonomySectionEntry[]>;
  sectionsByName: Map<string, PlannerTaxonomySectionEntry[]>;
}

function normalizePlacementNameForLookup(value?: string | null): string {
  return value?.trim().toLowerCase().replace(/\s+/g, ' ') ?? '';
}

function buildPlannerTaxonomyIndex(prefetch?: BatchPlannerPrefetch): PlannerTaxonomyIndex {
  const categoriesById = new Map<string, { categoryName: string }>();
  const categoryIdsByName = new Map<string, string[]>();
  const sectionsById = new Map<string, PlannerTaxonomySectionEntry>();
  const sectionsByCategoryId = new Map<string, PlannerTaxonomySectionEntry[]>();
  const sectionsByName = new Map<string, PlannerTaxonomySectionEntry[]>();
  ...
  for (const category of prefetch?.zendeskTaxonomy?.categories ?? []) {
    ...
    for (const section of category.sections ?? []) {
      ...
      const entry: PlannerTaxonomySectionEntry = {
        sectionId,
        sectionName,
        categoryId,
        categoryName
...
function resolvePlacementAgainstPlannerTaxonomy(
  placement: ProposalPlacementSuggestion | undefined,
  taxonomy: PlannerTaxonomyIndex
  ...
  if (!sectionId && sectionName) {
    ...
    let matchingSections: PlannerTaxonomySectionEntry[] = [];
    if (categoryId) {
  ...
  const resolved: ProposalPlacementSuggestion = {
...
function mergePlacementSuggestions(
  base: ProposalPlacementSuggestion | undefined,
  inferred: ProposalPlacementSuggestion | undefined
): ProposalPlacementSuggestion | undefined {
  if (!base && !inferred) {
    return undefined;
  }
  const merged: ProposalPlacementSuggestion = {
...
function hasDeterministicCreatePlacement(placement?: ProposalPlacementSuggestion): boolean {
  const categoryId = normalizePlacementScopeId(placement?.categoryId);
  const categoryName = normalizePromptString(placement?.categoryName);
  const sectionId = normalizePlacementScopeId(placement?.sectionId);
  const sectionName = normalizePromptString(placement?.sectionName);
...
function placementMatchesHint(
  placement: ProposalPlacementSuggestion | undefined,
  hint: ProposalPlacementSuggestion | undefined
): boolean {
  if (!hint) {
    return true;
  }
...
function buildPlacementCandidateKey(placement: ProposalPlacementSuggestion): string {
...
function collectStrongPlacementMatchesForPlanItem(
  item: Pick<BatchPlanItem, 'pbiIds' | 'targetTitle'>,
  plannerPrefetch: BatchPlannerPrefetch | undefined,
  taxonomy: PlannerTaxonomyIndex,
  hint: ProposalPlacementSuggestion | undefined
): Array<{ placement: ProposalPlacementSuggestion; score: number }> {
  if (!plannerPrefetch) {
    return [];
  }
  ...
  const matches: Array<{ placement: ProposalPlacementSuggestion; score: number }> = [];
...
function inferPlacementFromStrongArticleMatches(
  item: Pick<BatchPlanItem, 'pbiIds' | 'targetTitle'>,
  plannerPrefetch: BatchPlannerPrefetch | undefined,
  taxonomy: PlannerTaxonomyIndex,
  hint: ProposalPlacementSuggestion | undefined
...
function resolveDeterministicCreatePlanPlacement(params: {
  item: Pick<BatchPlanItem, 'action' | 'pbiIds' | 'targetTitle' | 'targetLocale' | 'suggestedPlacement'>;
  plannerPrefetch?: BatchPlannerPrefetch;
}): ProposalPlacementSuggestion | undefined {
  ...
  if (!hasDeterministicCreatePlacement(resolvedPlacement)) {
    const inferredPlacement = inferPlacementFromStrongArticleMatches(
      params.item,
      params.plannerPrefetch,
      taxonomy,
      resolvedPlacement
    );
    resolvedPlacement = resolvePlacementAgainstPlannerTaxonomy(
      mergePlacementSuggestions(resolvedPlacement, inferredPlacement),
      taxonomy
    );
  }
...
function doesPlacementMeetRequirement(
  actual?: {
    categoryId?: string;
    categoryName?: string;
    sectionId?: string;
...
function collectLinkedDiscoveryIds(params: {
  ...
  return dedupeIdList(
    params.discoveredWork
      .filter((item) => item.requiresPlanAmendment !== false)
      .filter((item) =>
        item.linkedPbiIds.some((pbiId) => linkedPbiIds.has(pbiId.trim()))
...
function mergeStructuredQuestions(...questionGroups: BatchAnalysisQuestion[][]): BatchAnalysisQuestion[] {
  const merged = new Map<string, MergeableBatchAnalysisQuestion>();
  const legacyKeys = new Map<string, string>();

  for (const question of questionGroups.flat()) {
    const mergeableQuestion = question as MergeableBatchAnalysisQuestion;
...
function collectBlockingUserInputQuestions(...questionGroups: BatchAnalysisQuestion[][]): BatchAnalysisQuestion[] {
  return mergeStructuredQuestions(...questionGroups)
    .filter((question) => isPendingRequiredUserInputQuestion(question));
}

function normalizeQuestionForMerge(question: MergeableBatchAnalysisQuestion): MergeableBatchAnalysisQuestion {
  return {
    ...question,
...
function mergeTwoStructuredQuestions(
  existing: MergeableBatchAnalysisQuestion,
  incoming: MergeableBatchAnalysisQuestion
): MergeableBatchAnalysisQuestion {
  const merged: MergeableBatchAnalysisQuestion = {
    ...existing,
...
function shouldUseIncomingQuestionField(
  question: MergeableBatchAnalysisQuestion,
  field: keyof QuestionMergeMeta
): boolean {
...
function stripQuestionMergeMeta(question: MergeableBatchAnalysisQuestion): BatchAnalysisQuestion {
  const { __mergeMeta: _ignored, ...normalizedQuestion } = question;
  return normalizedQuestion;
}

function normalizeQuestionLegacyKey(question: Pick<BatchAnalysisQuestion, 'prompt' | 'reason'>): string {
  return `${normalizeQuestionPromptKey(question.prompt)}|${normalizeQuestionPromptKey(question.reason)}`;
}

function normalizeQuestionPromptKey(value: string): string {
  return humanizeReadableText(value).trim().replace(/\s+/g, ' ').toLowerCase();
}

function dedupeIdList(values: string[] | undefined): string[] {
  return Array.from(
    new Set(
      (values ?? [])
        .map((value) => value.trim())
...
function isPendingRequiredUserInputQuestion(question: BatchAnalysisQuestion): boolean {
