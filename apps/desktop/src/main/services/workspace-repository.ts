...
import {
  ...
  type ArticleRelationCorpusExportRequest,
  type ArticleRelationCorpusExportResponse,
  type CoverageQueryRequest,
  type CoverageQueryResponse,
  type GraphQueryRequest,
  type GraphQueryResponse,
  type FeatureMapSummaryRequest,
  type FeatureMapSummaryResponse,
  type FeatureClusterLabelSource,
  type FeatureMapScopeSummary,
  type FeatureScopeRequest,
  type FeatureScopeResponse,
  type ArticleNeighborhoodRequest,
  type ArticleNeighborhoodResponse,
  type ArticleRelationDeleteRequest,
  type ArticleRelationFeedbackRecord,
  type ArticleRelationFeedbackRecordRequest,
  ArticleRelationFeedbackSource,
  ArticleRelationFeedbackType,
  ArticleRelationIndexStateStatus,
  type ArticleRelationIndexStats,
  type ArticleTaxonomySource,
  type KBScopeCatalogRecord,
  type KBScopeCatalogQuery,
  type KBScopeCatalogUpsertInput,
  type KBScopeDisplayNameInput,
  type KBScopeDisplayNameRecord,
  type KBScopeLabelSource,
  type KBScopeOverrideQuery,
  type KBScopeOverrideRecord,
  type KBScopeType,
  ...
  type BatchAnalysisFinalReviewDeltaRecord,
  type BatchAnalysisInspectionResponse,
  type BatchAnalysisIterationRecord,
  type BatchAnalysisReviewDeltaRecord,
  type BatchAnalysisStageRunRecord,
  type BatchPlanAmendment,
  type BatchAnalysisPlan,
  type BatchAnalysisSnapshotResponse,
  type BatchAnalysisStageStatus,
  type BatchAnalysisArtifactVerdict,
  type BatchAnalysisIterationStatus,
  type BatchAnalysisTimelineEntry,
  type BatchAnalysisTranscriptLink,
  type BatchDiscoveredWorkItem,
  type BatchFinalReview,
  type BatchPlanItem,
  type BatchPlanReview,
  type BatchWorkerExecutionReport,
  type PersistedAgentAnalysisRun,
  type PBIRecord,
  type PBIBatchAnalysisConfig,
  type PBIBatchAnalysisConfigInput,
  type PBIBatchContextResponse,
  type PBIBatchGuaranteedCreateArticle,
  type PBIBatchGuaranteedCreateConflict,
  type PBIBatchGuaranteedCreateConflictMatch,
  type PBIBatchGuaranteedEditFamily,
  type PBIBatchGuaranteedEditLocaleVariant,
  type PBIBatchRecord,
  ...
  type WorkspaceBatchAnalysisRepairSummary,
  ...
  type ProposalReviewDeleteResponse,
...
import {
  ArticleRelationsV2ExportDocumentError,
  ArticleRelationsV2ExportService
} from './article-relations-v2/export-service';
import { ArticleRelationsV2FeatureMapService } from './article-relations-v2/feature-map-service';
import { ArticleRelationsV2IndexBuilder } from './article-relations-v2/index-builder';
import { ArticleRelationsV2IndexDb } from './article-relations-v2/index-db';
import { ArticleRelationsV2QueryService } from './article-relations-v2/query-service';
import { ArticleRelationsV2RelationOrchestrator } from './article-relations-v2/relation-orchestrator';
import {
  ARTICLE_RELATIONS_V2_ENGINE_VERSION,
  ARTICLE_RELATIONS_V2_INDEX_DB_RELATIVE_PATH,
  ARTICLE_RELATIONS_V2_LEGACY_ENGINE_VERSION,
  type ArticleRelationsV2IndexBuildResult
} from './article-relations-v2/types';
...
const ARTICLE_TAXONOMY_SOURCE_VALUES: ArticleTaxonomySource[] = [
  'zendesk_article',
...
interface ArticleTaxonomyResolution {
  sectionId?: string;
  categoryId?: string;
...
interface ArticleTaxonomyInferenceCandidate {
  sectionId?: string;
  categoryId?: string;
  confidence: number;
  note?: string;
...
interface LegacyBatchAnalysisRunRow {
  id: string;
  workspaceId: string;
  batchId: string;
...
interface BatchAnalysisStageRunRow {
  ...
  iterationId: string;
  iteration: number;
  stage: BatchAnalysisStageRunRecord['stage'];
  role: BatchAnalysisStageRunRecord['role'];
  attempt: number;
  retryType: string | null;
  sessionReusePolicy: BatchAnalysisStageRunRecord['sessionReusePolicy'] | null;
  ...
  rawOutputJson: string | null;
  message: string | null;
  parseable: number | null;
  usedTranscriptRecovery: number | null;
  initialCandidateCount: number | null;
  transcriptCandidateCount: number | null;
...
interface ArticleRelationGraphNodeRow {
  familyId: string;
  title: string;
...
interface FeatureMapRelationRow extends ArticleRelationDbRow {
...
interface KBScopeCatalogDbRow extends KBScopeCatalogRecord {}

interface KBScopeOverrideDbRow extends KBScopeOverrideRecord {}

interface ArticleRelationsV2CoverageSourceStateRow {
  familyId: string;
  localeVariantId: string;
  revisionId: string;
  contentHash: string;
...
interface ArticleRelationsV2CoverageMainIndexStateRow {
  localeVariantId: string;
  familyId: string;
  revisionId: string;
  contentHash: string;
...
interface ArticleRelationsV2CoverageFreshnessResult {
  sourceStates: ArticleRelationsV2CoverageSourceStateRow[];
  requiresRebuild: boolean;
  hasUsableIndex: boolean;
  reason: string;
}

interface ArticleRelationsV2RefreshPreparedIndex {
...
export class WorkspaceRepository {
  ...
  private readonly articleRelationsV2RelationOrchestrator: ArticleRelationsV2RelationOrchestrator;
  ...
  constructor(private readonly workspaceRoot: string) {
    ...
    this.articleRelationsV2FeatureMapService = new ArticleRelationsV2FeatureMapService({
    ...
    this.articleRelationsV2QueryService = new ArticleRelationsV2QueryService();
    this.articleRelationsV2RelationOrchestrator = new ArticleRelationsV2RelationOrchestrator(this.articleRelationsV2QueryService);
  }

  private normalizeAgentModelId(modelId?: string | null): string | undefined {
  ...
  private normalizeAcpModelId(modelId?: string | null): string | undefined {
  ...
  async withWorkspaceDb<T>(
    workspaceId: string,
    handler: (workspaceDb: SQLite) => Promise<T> | T
  ): Promise<T> {
  ...
  async listArticleFamilies(workspaceId: string): Promise<ArticleFamilyRecord[]> {
    ...
    const workspaceDb = this.openWorkspaceDbWithRecovery(path.join(workspace.path, '.meta', DEFAULT_DB_FILE));
    try {
      return workspaceDb.all<ArticleFamilyDbRow>(
        `SELECT ${ARTICLE_FAMILY_SELECT_COLUMNS}
         FROM article_families
         WHERE workspace_id = @workspaceId
         ORDER BY title`,
        { workspaceId }
      ).map(mapArticleFamilyRow);
    } finally {
      workspaceDb.close();
    }
  }
  ...
  async updateArticleFamily(payload: ArticleFamilyUpdateRequest): Promise<ArticleFamilyRecord> {
    ...
    const workspaceDb = this.openWorkspaceDbWithRecovery(path.join(workspace.path, '.meta', DEFAULT_DB_FILE));
    try {
      const existingRow = workspaceDb.get<ArticleFamilyDbRow>(
        `SELECT ${ARTICLE_FAMILY_SELECT_COLUMNS}
         FROM article_families
         WHERE id = @familyId AND workspace_id = @workspaceId`,
      ...
      if (familyMetadataChanged) {
        try {
          ...
          logger.warn('workspace-repository.updateArticleFamily stale-mark failed', {
            workspaceId: payload.workspaceId,
            familyId: payload.familyId,
            message: error instanceof Error ? error.message : String(error)
          });
  ...
  async getKbSectionParentCategory(workspaceId: string, sectionId: string): Promise<string | undefined> {
    const normalizedSectionId = normalizeFeatureMapScopeId(sectionId);
    if (!normalizedSectionId) {
      return undefined;
    }

    return this.withWorkspaceDb(workspaceId, (workspaceDb) => (
      this.getKbSectionParentCategoryInDb(workspaceDb, workspaceId, normalizedSectionId)
    ));
  }

  async resolveEffectiveArticleTaxonomyPlacement(
    workspaceId: string,
    familyId: string,
    options: {
  ...
  async reconcileEffectiveArticleTaxonomyPlacements(
    workspaceId: string,
    options: {
      familyIds?: string[];
      allowInference?: boolean;
    ...
    try {
      ...
      const results: ArticleFamilyRecord[] = [];
      ...
      if (changedFamilyIds.length > 0) {
        try {
          ...
          logger.warn('workspace-repository.reconcileEffectiveArticleTaxonomyPlacements stale-mark failed', {
            workspaceId,
  ...
  async getActiveTemplatePackByType(
    workspaceId: string,
    templateType: TemplatePackType
  ): Promise<TemplatePackDetail | null> {
    const workspace = await this.getWorkspace(workspaceId);
    const workspaceDb = this.openWorkspaceDbWithRecovery(path.join(workspace.path, '.meta', DEFAULT_DB_FILE));
    try {
      await this.ensureDefaultTemplatePacks(workspaceId, workspaceDb);
    ...
    try {
      ...
      if (variantMetadataChanged) {
        try {
          ...
          logger.warn('workspace-repository.updateLocaleVariant stale-mark failed', {
            workspaceId: payload.workspaceId,
  ...
  async reconcileSyncedLocaleVariants(
    ...
    const workspaceDb = this.openWorkspaceDbWithRecovery(path.join(workspace.path, '.meta', DEFAULT_DB_FILE));
    try {
      const affectedFamilies = workspaceDb.all<{ familyId: string }>(
      ...
      if (!remoteFamilyExternalKeys.length) {
        ...
        const obsoleteBranches = workspaceDb.all<{ id: string; localeVariantId: string }>(
          `SELECT db.id, db.locale_variant_id as localeVariantId
        ...
        if (affectedFamilies.length > 0) {
          try {
            ...
            logger.warn('workspace-repository.reconcileSyncedLocaleVariants stale-mark failed', {
              workspaceId,
              locale,
              message: error instanceof Error ? error.message : String(error)
            });
  ...
  async createRevision(payload: RevisionCreateRequest): Promise<RevisionRecord> {
    ...
    const workspaceDb = this.openWorkspaceDbWithRecovery(path.join(workspace.path, '.meta', DEFAULT_DB_FILE));
    try {
      const variant = workspaceDb.get<{ id: string; familyId: string }>(
      ...
      if (payload.revisionType === RevisionState.LIVE) {
        try {
          this.markArticleRelationFamiliesStale(workspaceDb, payload.workspaceId, [variant.familyId]);
        } catch (error) {
          logger.warn('workspace-repository.createRevision stale-mark failed', {
            workspaceId: payload.workspaceId,
  ...
  async updateRevision(payload: RevisionUpdateRequest): Promise<RevisionRecord> {
    ...
    try {
      ...
      if (variant && (revisionType === RevisionState.LIVE || existing.revisionType === RevisionState.LIVE)) {
        try {
          this.markArticleRelationFamiliesStale(workspaceDb, payload.workspaceId, [variant.familyId]);
        } catch (error) {
          logger.warn('workspace-repository.updateRevision stale-mark failed', {
            workspaceId: payload.workspaceId,
            familyId: variant.familyId,
            localeVariantId: existing.localeVariantId,
            revisionId: payload.revisionId,
  ...
  async deleteRevision(workspaceId: string, revisionId: string): Promise<void> {
    ...
    try {
      const existing = workspaceDb.get<{
        ...
        `SELECT r.locale_variant_id as localeVariantId,
      ...
      if (existing?.revisionType === RevisionState.LIVE) {
        try {
          this.markArticleRelationFamiliesStale(workspaceDb, workspaceId, [existing.familyId]);
        } catch (error) {
          logger.warn('workspace-repository.deleteRevision stale-mark failed', {
            workspaceId,
            familyId: existing.familyId,
            localeVariantId: existing.localeVariantId,
            revisionId,
  ...
  async searchArticles(workspaceId: string, payload: SearchPayload): Promise<SearchResponse> {
    ...
    try {
      ...
      const revisionsByVariant = new Map<string, Array<{
        revisionId: string;
        localeVariantId: string;
        revisionNumber: number;
        revisionType: RevisionState;
      ...
      for (const family of families) {
        ...
        for (const variant of familyVariants) {
          ...
          let matchSource: SearchSourceMatch = {
  ...
  async getArticleRelationsStatus(workspaceId: string): Promise<ArticleRelationRefreshStatusResponse> {
    ...
    try {
      ...
      const latestRun = workspaceDb.get<{
        ...
        stale_document_count: number | null;
        degraded_mode: number | null;
      ...
      const indexCounts = this.loadArticleRelationIndexStateCounts(
        workspaceDb,
        workspaceId,
      ...
      const summary: ArticleRelationSummary = {
        ...
        engineVersion: latestRun?.engine_version ?? ARTICLE_RELATIONS_V2_LEGACY_ENGINE_VERSION,
      ...
      return {
        ...
        latestRun: latestRun
          ? {
              ...
              degradedMode: Boolean(latestRun.degraded_mode),
              thresholdsUsed: safeParseJson<Record<string, number>>(latestRun.thresholds_json) ?? undefined,
    ...
    try {
      ...
      try {
        ...
        this.insertArticleRelationFeedback(workspaceDb, {
          ...
          leftFamilyId: pair.leftFamilyId,
          rightFamilyId: pair.rightFamilyId,
          feedbackType: ArticleRelationFeedbackType.ADD,
          source: ArticleRelationFeedbackSource.MANUAL_RELATION,
          note: payload.note
        }, now);
      ...
      try {
        ...
        logger.warn('workspace-repository.upsertManualArticleRelation stale-mark failed', {
          workspaceId: payload.workspaceId,
          leftFamilyId: pair.leftFamilyId,
          rightFamilyId: pair.rightFamilyId,
          message: error instanceof Error ? error.message : String(error)
    ...
    try {
      ...
      try {
        ...
        this.insertArticleRelationFeedback(workspaceDb, {
          ...
          leftFamilyId: pair.leftFamilyId,
          rightFamilyId: pair.rightFamilyId,
          feedbackType: ArticleRelationFeedbackType.REMOVE,
          source: ArticleRelationFeedbackSource.MANUAL_RELATION
        }, now);
        workspaceDb.exec('COMMIT');
      } catch (error) {
      ...
      try {
        ...
        logger.warn('workspace-repository.deleteArticleRelation stale-mark failed', {
          workspaceId: payload.workspaceId,
          leftFamilyId: pair.leftFamilyId,
          rightFamilyId: pair.rightFamilyId,
          message: error instanceof Error ? error.message : String(error)
  ...
  async exportArticleRelationCorpus(
    workspaceId: string,
    payload: Omit<ArticleRelationCorpusExportRequest, 'workspaceId'> = {}
  ): Promise<ArticleRelationCorpusExportResponse> {
    const workspace = await this.getWorkspace(workspaceId);
  ...
  async rebuildArticleRelationCoverageIndex(
    workspaceId: string,
    options?: { forceFullRebuild?: boolean }
  ): Promise<ArticleRelationsV2IndexBuildResult> {
    const workspace = await this.getWorkspace(workspaceId);
    const settings = await this.getWorkspaceSettings(workspaceId);
    await this.ensureWorkspaceDb(workspace.path);
    const workspaceDb = this.openWorkspaceDbWithRecovery(path.join(workspace.path, '.meta', DEFAULT_DB_FILE));
  ...
  async queryArticleRelationCoverage(request: CoverageQueryRequest): Promise<CoverageQueryResponse> {
    ...
    try {
      ...
      if (freshness.requiresRebuild) {
        try {
          ...
          logger.warn('workspace-repository.queryArticleRelationCoverage using stale index fallback', {
            workspaceId: request.workspaceId,
  ...
  async queryArticleRelationGraph(request: GraphQueryRequest): Promise<GraphQueryResponse> {
    const workspace = await this.getWorkspace(request.workspaceId);
    await this.ensureWorkspaceDb(workspace.path);
    const workspaceDb = this.openWorkspaceDbWithRecovery(path.join(workspace.path, '.meta', DEFAULT_DB_FILE));
    ...
    try {
      ...
      const rows = workspaceDb.all<ArticleRelationDbRow>(
        `SELECT
         ...
         WHERE ${where.join('\n           AND ')}
         ORDER BY r.strength_score DESC, r.updated_at DESC
         LIMIT ${Math.max(limitNodes * 8, 48)}`,
        params
      );
      ...
      const nodeRows = familyIds.size > 0
        ? workspaceDb.all<ArticleRelationGraphNodeRow>(
            `SELECT id as familyId,
                    ...
                    category_id as categoryId
               FROM article_families
              WHERE id IN (${buildRelationCoverageInClause('family', Array.from(familyIds), params as Record<string, string>)})`,
            params
          )
        : [];
  ...
  async getArticleRelationFeatureMapSummary(
  ...
  async getArticleRelationFeatureScope(
  ...
  async getArticleRelationNeighborhood(
  ...
  async recordArticleRelationFeedback(
    payload: ArticleRelationFeedbackRecordRequest
  ): Promise<ArticleRelationFeedbackRecord> {
    const workspace = await this.getWorkspace(payload.workspaceId);
    await this.ensureWorkspaceDb(workspace.path);
    const workspaceDb = this.openWorkspaceDbWithRecovery(path.join(workspace.path, '.meta', DEFAULT_DB_FILE));
    ...
    try {
      ...
      const record = this.insertArticleRelationFeedback(workspaceDb, {
        ...
        source: payload.source ?? ArticleRelationFeedbackSource.UI,
        note: payload.note
      }, now);
      ...
      if (shouldReevaluateFromRelationFeedback(payload.feedbackType)) {
        try {
          ...
          logger.warn('workspace-repository.recordArticleRelationFeedback stale-mark failed', {
            workspaceId: payload.workspaceId,
            leftFamilyId: pair.leftFamilyId,
            rightFamilyId: pair.rightFamilyId,
            feedbackType: payload.feedbackType,
  ...
  private async exportArticleRelationCorpusWithDb(input: {
    workspaceId: string;
    workspacePath: string;
    ...
    try {
      ...
      if (error instanceof ArticleRelationsV2ExportDocumentError) {
  ...
  private async rebuildArticleRelationCoverageIndexWithDb(input: {
    workspaceId: string;
    workspacePath: string;
    enabledLocales: string[];
    workspaceDb: ReturnType<WorkspaceRepository['openWorkspaceDbWithRecovery']>;
    ...
    const builder = new ArticleRelationsV2IndexBuilder(this.getArticleRelationsV2IndexDb(input.workspacePath));
    builder.rebuild({
      workspaceId: input.workspaceId,
      workspaceDb: input.workspaceDb,
  ...
  private async evaluateArticleRelationCoverageIndexFreshness(input: {
    workspaceId: string;
    workspacePath: string;
    enabledLocales: string[];
    workspaceDb: ReturnType<WorkspaceRepository['openWorkspaceDbWithRecovery']>;
  }): Promise<ArticleRelationsV2CoverageFreshnessResult> {
    const rawSourceStates = this.loadArticleRelationCoverageSourceStates(
      input.workspaceId,
    ...
    let derivedStates: Array<{ localeVariantId: string; familyId: string; revisionId: string; contentHash: string }> = [];
    let derivedDocumentCount = 0;
    let derivedEngineVersion = ARTICLE_RELATIONS_V2_ENGINE_VERSION;
    ...
    try {
      ...
      logger.warn('workspace-repository.evaluateArticleRelationCoverageIndexFreshness index open failed', {
        workspaceId: input.workspaceId,
        message: error instanceof Error ? error.message : String(error)
      });
      return {
    ...
    if (derivedEngineVersion !== ARTICLE_RELATIONS_V2_ENGINE_VERSION) {
      return {
        sourceStates: rawSourceStates,
        requiresRebuild: true,
        hasUsableIndex: false,
    ...
    const mainIndexStates = input.workspaceDb.all<ArticleRelationsV2CoverageMainIndexStateRow>(
    ...
    for (const sourceState of rawSourceStates) {
      ...
      if (mainState.status !== ArticleRelationIndexStateStatus.INDEXED) {
        return {
          sourceStates: rawSourceStates,
          requiresRebuild: true,
          hasUsableIndex,
      ...
      if (
        mainState.familyId !== sourceState.familyId
        || mainState.revisionId !== sourceState.revisionId
        || mainState.engineVersion !== ARTICLE_RELATIONS_V2_ENGINE_VERSION
    ...
    let exportResponse: ArticleRelationCorpusExportResponse;
    try {
    ...
    const sourceStates = exportResponse.documents.map<ArticleRelationsV2CoverageSourceStateRow>((document) => ({
    ...
    for (const sourceState of sourceStates) {
      ...
      if (
        derivedState.familyId !== sourceState.familyId
  ...
  private getArticleRelationsV2IndexDb(workspacePath: string): ArticleRelationsV2IndexDb {
    return new ArticleRelationsV2IndexDb(this.getArticleRelationsV2IndexDbPath(workspacePath));
  }

  private getArticleRelationsV2IndexDbPath(workspacePath: string): string {
    return path.join(workspacePath, ARTICLE_RELATIONS_V2_INDEX_DB_RELATIVE_PATH);
  }
  ...
  private loadArticleRelationCoverageSourceStates(
    ...
    workspaceDb: ReturnType<WorkspaceRepository['openWorkspaceDbWithRecovery']>,
    familyIds?: string[]
  ): ArticleRelationsV2CoverageSourceStateRow[] {
    ...
    return workspaceDb.all<ArticleRelationsV2CoverageSourceStateRow>(
      `SELECT
         af.id as familyId,
  ...
  private recordArticleRelationExportError(
    workspaceDb: ReturnType<WorkspaceRepository['openWorkspaceDbWithRecovery']>,
    workspaceId: string,
    error: ArticleRelationsV2ExportDocumentError
  ): void {
    workspaceDb.run(
         ...
         last_error = excluded.last_error`,
      {
        ...
        engineVersion: ARTICLE_RELATIONS_V2_ENGINE_VERSION,
        status: ArticleRelationIndexStateStatus.ERROR,
        lastError: error.message
      }
    );
  }

  private async prepareArticleRelationCoverageIndexForRefresh(input: {
    workspaceId: string;
    workspacePath: string;
    enabledLocales: string[];
    workspaceDb: ReturnType<WorkspaceRepository['openWorkspaceDbWithRecovery']>;
  }): Promise<ArticleRelationsV2RefreshPreparedIndex> {
    ...
    if (freshness.requiresRebuild) {
      try {
        ...
        degradedMode = true;
        logger.warn('workspace-repository.refreshArticleRelations using stale index fallback', {
          workspaceId: input.workspaceId,
          reason: freshness.reason,
          message: error instanceof Error ? error.message : String(error)
        });
  ...
  private loadArticleRelationIndexStateCounts(
    workspaceDb: ReturnType<WorkspaceRepository['openWorkspaceDbWithRecovery']>,
    workspaceId: string,
    enabledLocales: string[]
  ): { indexedDocumentCount: number; staleDocumentCount: number } {
    ...
    const localeVariantPlaceholders = buildRelationCoverageInClause(
      'countLocaleVariant',
  ...
  private upsertKbScopeCatalogEntriesInDb(
    workspaceDb: ReturnType<WorkspaceRepository['openWorkspaceDbWithRecovery']>,
    workspaceId: string,
    entries: KBScopeCatalogUpsertInput[],
    updatedAtUtc: string
  ): KBScopeCatalogRecord[] {
    const recordsByKey = new Map<string, KBScopeCatalogRecord>();
    ...
    for (const entry of entries) {
      ...
      const record: KBScopeCatalogRecord = {
        workspaceId,
        scopeType,
        scopeId,
        parentScopeId,
  ...
  private listKbScopeCatalogEntriesInDb(
    workspaceDb: ReturnType<WorkspaceRepository['openWorkspaceDbWithRecovery']>,
    workspaceId: string,
    query: KBScopeCatalogQuery = {}
  ): KBScopeCatalogRecord[] {
    ...
    const scopeIds = Array.from(new Set((query.scopeIds ?? []).map((value) => normalizeFeatureMapScopeId(value)).filter(Boolean))) as string[];
    if (scopeIds.length > 0) {
      where.push(`scope_id IN (${buildRelationCoverageInClause('scopeId', scopeIds, params)})`);
    }

    return workspaceDb.all<KBScopeCatalogDbRow>(
      `SELECT workspace_id as workspaceId,
        ...
        WHERE ${where.join('\n          AND ')}
        ORDER BY scope_type ASC, display_name COLLATE NOCASE ASC, scope_id ASC`,
      params
    ).map((row) => ({
      ...row,
  ...
  private listKbScopeOverridesInDb(
    workspaceDb: ReturnType<WorkspaceRepository['openWorkspaceDbWithRecovery']>,
    workspaceId: string,
    query: KBScopeOverrideQuery = {}
  ): KBScopeOverrideRecord[] {
    const where = ['workspace_id = @workspaceId'];
    const params: Record<string, string> = { workspaceId };
    ...
    return workspaceDb.all<KBScopeOverrideDbRow>(
      `SELECT id,
              workspace_id as workspaceId,
              scope_type as scopeType,
              scope_id as scopeId,
        ...
        WHERE ${where.join('\n          AND ')}
        ORDER BY scope_type ASC, updated_at DESC, scope_id ASC`,
      params
    ).map((row) => ({
      ...row,
  ...
  private resolveKbScopeDisplayNamesInDb(
    workspaceDb: ReturnType<WorkspaceRepository['openWorkspaceDbWithRecovery']>,
    workspaceId: string,
    scopes: KBScopeDisplayNameInput[]
  ): KBScopeDisplayNameRecord[] {
    const normalizedScopes: KBScopeDisplayNameInput[] = [];
    ...
    const catalogByKey = new Map(
      this.listKbScopeCatalogEntriesInDb(workspaceDb, workspaceId, { scopeIds })
        .map((record) => [buildFeatureMapBucketKey(record.scopeType, record.scopeId), record])
    );
    const overridesByKey = new Map(
      this.listKbScopeOverridesInDb(workspaceDb, workspaceId, { scopeIds })
        .map((record) => [buildFeatureMapBucketKey(record.scopeType, record.scopeId), record])
    );
    ...
    return normalizedScopes.map((scope) => {
      ...
      const labelSource: KBScopeLabelSource = overrideName
        ? 'override'
  ...
  private async loadArticleRelationIndexStats(
    workspacePath: string,
    workspaceId: string
  ): Promise<ArticleRelationIndexStats | undefined> {
    ...
    try {
      ...
      logger.warn('workspace-repository.loadArticleRelationIndexStats failed', {
        workspaceId,
        message: error instanceof Error ? error.message : String(error)
      });
      return undefined;
  ...
  private insertArticleRelationFeedback(
    workspaceDb: ReturnType<WorkspaceRepository['openWorkspaceDbWithRecovery']>,
    input: {
      workspaceId: string;
      leftFamilyId: string;
    ...
    createdAtUtc: string
  ): ArticleRelationFeedbackRecord {
    const record: ArticleRelationFeedbackRecord = {
      id: randomUUID(),
      workspaceId: input.workspaceId,
    ...
    workspaceDb.run(
      `INSERT INTO article_relation_feedback (
  ...
  private markArticleRelationNeighborhoodStale(
    workspaceDb: ReturnType<WorkspaceRepository['openWorkspaceDbWithRecovery']>,
    workspaceId: string,
    familyIds: string[]
  ): void {
  ...
  private loadArticleRelationNeighborhoodFamilyIds(
    workspaceDb: ReturnType<WorkspaceRepository['openWorkspaceDbWithRecovery']>,
    workspaceId: string,
    familyIds: string[]
  ): string[] {
    ...
    const relatedFamilies = workspaceDb.all<{ familyId: string }>(
      `SELECT DISTINCT CASE
       ...
       FROM article_relations
       WHERE workspace_id = @workspaceId
         AND (left_family_id IN (${placeholders}) OR right_family_id IN (${placeholders}))`,
      params
    );
  ...
  private markArticleRelationFamiliesStale(
    workspaceDb: ReturnType<WorkspaceRepository['openWorkspaceDbWithRecovery']>,
    workspaceId: string,
    familyIds: string[]
  ): void {
    ...
    const scopedParams: Record<string, string> = { ...params };
    const keepLocaleVariantPlaceholders = buildRelationCoverageInClause(
      'keepLocaleVariant',
      sourceStates.map((row) => row.localeVariantId),
      scopedParams
    );
    workspaceDb.run(
      `DELETE FROM article_relation_index_state
       WHERE workspace_id = @workspaceId
         AND family_id IN (${familyPlaceholders})
         AND locale_variant_id NOT IN (${keepLocaleVariantPlaceholders})`,
      scopedParams
    );
  ...
  async recordBatchAnalysisStageRun(params: {
    ...
    attempt?: number;
    retryType?: string;
    sessionReusePolicy?: BatchAnalysisStageRunRecord['sessionReusePolicy'];
    ...
    try {
      ...
      const attempt = Number.isFinite(params.attempt)
        ...
        : (workspaceDb.get<{ nextAttempt: number }>(
            `SELECT COALESCE(MAX(attempt), 0) + 1 as nextAttempt
      ...
      workspaceDb.run(
        `INSERT INTO batch_analysis_stage_runs (
  ...
  async getLatestBatchAnalysisRun(workspaceId: string, batchId: string): Promise<PersistedAgentAnalysisRun | null> {
    ...
    const workspaceDb = this.openWorkspaceDbWithRecovery(path.join(workspace.path, '.meta', DEFAULT_DB_FILE));
    try {
      const latestStageRun = workspaceDb.get<BatchAnalysisStageRunRow>(
        `SELECT id,
                workspace_id as workspaceId,
                batch_id as batchId,
                iteration_id as iterationId,
  ...
  async createBatchAnalysisIteration(params: {
    ...
    try {
      const now = new Date().toISOString();
      const row = workspaceDb.get<{ nextIteration: number }>(
        `SELECT COALESCE(MAX(iteration), 0) + 1 as nextIteration
           FROM batch_analysis_iterations
          WHERE workspace_id = @workspaceId AND batch_id = @batchId`,
        { workspaceId: params.workspaceId, batchId: params.batchId }
      );
      ...
      workspaceDb.run(
        `INSERT INTO batch_analysis_iterations (
  ...
  async updateBatchAnalysisIteration(params: {
    ...
    try {
      ...
      workspaceDb.run(
        `UPDATE batch_analysis_iterations
  ...
  async recordBatchWorkerExecutionReport(report: BatchWorkerExecutionReport): Promise<BatchWorkerExecutionReport> {
    const workspace = await this.getWorkspace(report.workspaceId);
    await this.ensureWorkspaceDb(workspace.path);
    const workspaceDb = this.openWorkspaceDbWithRecovery(path.join(workspace.path, '.meta', DEFAULT_DB_FILE));
    try {
      workspaceDb.run(
        `INSERT INTO batch_analysis_worker_reports (
      ...
      for (const item of report.discoveredWork) {
        workspaceDb.run(
          `INSERT INTO batch_analysis_discovered_work (
  ...
  async recordBatchAnalysisPlan(plan: BatchAnalysisPlan): Promise<BatchAnalysisPlan> {
    const workspace = await this.getWorkspace(plan.workspaceId);
    await this.ensureWorkspaceDb(workspace.path);
    const workspaceDb = this.openWorkspaceDbWithRecovery(path.join(workspace.path, '.meta', DEFAULT_DB_FILE));
    try {
      workspaceDb.run(
        `INSERT INTO batch_analysis_plans (
      ...
      for (const item of plan.items) {
        workspaceDb.run(
          `INSERT INTO batch_analysis_plan_items (
  ...
  async recordBatchPlanReview(review: BatchPlanReview): Promise<BatchPlanReview> {
    const workspace = await this.getWorkspace(review.workspaceId);
    await this.ensureWorkspaceDb(workspace.path);
    const workspaceDb = this.openWorkspaceDbWithRecovery(path.join(workspace.path, '.meta', DEFAULT_DB_FILE));
    try {
      workspaceDb.run(
        `INSERT INTO batch_analysis_reviews (
  ...
  async updateBatchAnalysisPlanItemStatuses(params: {
    workspaceId: string;
    planId: string;
    statuses: Array<{ planItemId: string; executionStatus: BatchPlanItem['executionStatus'] }>;
  }): Promise<void> {
    ...
    try {
      const statement = workspaceDb.prepare(
        `UPDATE batch_analysis_plan_items
  ...
  async recordBatchPlanAmendment(amendment: BatchPlanAmendment): Promise<BatchPlanAmendment> {
    const workspace = await this.getWorkspace(amendment.workspaceId);
    await this.ensureWorkspaceDb(workspace.path);
    const workspaceDb = this.openWorkspaceDbWithRecovery(path.join(workspace.path, '.meta', DEFAULT_DB_FILE));
    try {
      workspaceDb.run(
        `INSERT INTO batch_analysis_amendments (
  ...
  async recordBatchFinalReview(review: BatchFinalReview): Promise<BatchFinalReview> {
    const workspace = await this.getWorkspace(review.workspaceId);
    await this.ensureWorkspaceDb(workspace.path);
    const workspaceDb = this.openWorkspaceDbWithRecovery(path.join(workspace.path, '.meta', DEFAULT_DB_FILE));
    try {
      workspaceDb.run(
        `INSERT INTO batch_analysis_final_reviews (
          id, workspace_id, batch_id, iteration_id, iteration, stage, role, verdict, summary,
  ...
  async recordBatchAnalysisStageEvent(event: BatchAnalysisStageEventRecord): Promise<BatchAnalysisStageEventRecord> {
    const workspace = await this.getWorkspace(event.workspaceId);
    await this.ensureWorkspaceDb(workspace.path);
    const workspaceDb = this.openWorkspaceDbWithRecovery(path.join(workspace.path, '.meta', DEFAULT_DB_FILE));
    try {
      workspaceDb.run(
        `INSERT INTO batch_analysis_stage_events (
  ...
  async getBatchAnalysisSnapshot(workspaceId: string, batchId: string): Promise<BatchAnalysisSnapshotResponse> {
    const workspace = await this.getWorkspace(workspaceId);
    await this.ensureWorkspaceDb(workspace.path);
    const workspaceDb = this.openWorkspaceDbWithRecovery(path.join(workspace.path, '.meta', DEFAULT_DB_FILE));
    try {
  ...
  async getBatchAnalysisInspection(workspaceId: string, batchId: string): Promise<BatchAnalysisInspectionResponse> {
    const workspace = await this.getWorkspace(workspaceId);
    await this.ensureWorkspaceDb(workspace.path);
    const workspaceDb = this.openWorkspaceDbWithRecovery(path.join(workspace.path, '.meta', DEFAULT_DB_FILE));
    try {
      ...
      const reviewDeltas: BatchAnalysisReviewDeltaRecord[] = reviews
      ...
      const finalReviewReworkPlans: BatchAnalysisFinalReviewDeltaRecord[] = finalReviews
      ...
      const transcriptLinks: BatchAnalysisTranscriptLink[] = [
        ...iterations.flatMap((iteration) => iteration.sessionId ? [{
          artifactType: 'iteration' as const,
        ...
        ...workerReports.flatMap((report) => report.sessionId ? [{
          artifactType: 'worker_report' as const,
        ...
        ...amendments.map((amendment) => ({
          artifactType: 'amendment' as const,
          artifactId: amendment.id,
          iterationId: amendment.iterationId,
          stage: 'worker_discovery_review' as const,
          role: 'planner' as const,
          createdAtUtc: amendment.createdAtUtc
        })),
        ...finalReviews.flatMap((review) => review.sessionId ? [{
          artifactType: 'final_review' as const,
          artifactId: review.id,
          iterationId: review.iterationId,
          iteration: review.iteration,
          stage: review.stage,
        ...
        ...stageRuns.flatMap((run) => run.localSessionId ? [{
          artifactType: 'stage_run' as const,
          artifactId: run.id,
      ...
      const timeline: BatchAnalysisTimelineEntry[] = [
  ...
  async getBatchAnalysisRuntimeStatus(workspaceId: string, batchId: string): Promise<BatchAnalysisRuntimeStatus | null> {
    const workspace = await this.getWorkspace(workspaceId);
    await this.ensureWorkspaceDb(workspace.path);
    const workspaceDb = this.openWorkspaceDbWithRecovery(path.join(workspace.path, '.meta', DEFAULT_DB_FILE));
    try {
  ...
  async getBatchAnalysisEventStream(workspaceId: string, batchId: string, limit = 100): Promise<BatchAnalysisEventStreamResponse> {
    const workspace = await this.getWorkspace(workspaceId);
    await this.ensureWorkspaceDb(workspace.path);
    const workspaceDb = this.openWorkspaceDbWithRecovery(path.join(workspace.path, '.meta', DEFAULT_DB_FILE));
    try {
  ...
  async deleteProposalReview(workspaceId: string, proposalId: string): Promise<ProposalReviewDeleteResponse> {
    const workspace = await this.getWorkspace(workspaceId);
    const workspaceDb = this.openWorkspaceDbWithRecovery(path.join(workspace.path, '.meta', DEFAULT_DB_FILE));
    try {
      ...
      await fs.rm(path.join(workspace.path, 'proposals', proposalId), { recursive: true, force: true });

      const remainingRows = workspaceDb.all<ProposalDbRow>(`
        SELECT p.id,
               p.workspace_id as workspaceId,
               p.batch_id as batchId,
               p.action,
  ...
  async getPBIBatchAnalysisState(
    workspaceId: string,
    batchId: string
  ): Promise<{
    ...
    const workspaceDb = this.openWorkspaceDbWithRecovery(path.join(workspace.path, '.meta', DEFAULT_DB_FILE));
    try {
      const row = workspaceDb.get<{ analysisConfigJson: string | null }>(
        `SELECT analysis_config_json as analysisConfigJson
         FROM pbi_batches
         WHERE id = @batchId AND workspace_id = @workspaceId
         LIMIT 1`,
        { batchId, workspaceId }
  ...
  async setPBIBatchAnalysisConfig(
    workspaceId: string,
    batchId: string,
    input: PBIBatchAnalysisConfigInput
  ): Promise<{
    analysisConfig: PBIBatchAnalysisConfig;
    guaranteedCreateConflicts: PBIBatchGuaranteedCreateConflict[];
  }> {
    ...
    try {
      ...
      const normalizedConfig = await this.resolvePBIBatchAnalysisConfigInput(
        workspace,
        workspaceDb,
        input
      );
  ...
  async getBatchContext(workspaceId: string, batchId: string): Promise<PBIBatchContextResponse | null> {
    const batch = await this.getPBIBatch(workspaceId, batchId);
    if (!batch) {
      return null;
    }
  ...
  private async resolvePBIBatchAnalysisConfigInput(
    workspace: WorkspaceRecord,
    workspaceDb: ReturnType<WorkspaceRepository['openWorkspaceDbWithRecovery']>,
    input: PBIBatchAnalysisConfigInput
  ): Promise<PBIBatchAnalysisConfig> {
    const guaranteedEditFamiliesById = new Map<string, PBIBatchGuaranteedEditFamily>();
    ...
    for (const selection of editSelections) {
      ...
      const family = requestedFamilyId
        ? workspaceDb.get<{
            ...
            `SELECT id as familyId, title as familyTitle, section_id as sectionId, category_id as categoryId
             FROM article_families
             WHERE id = @familyId
             LIMIT 1`,
        ...
        : requestedLocaleVariantId
          ? workspaceDb.get<{
              ...
              `SELECT af.id as familyId, af.title as familyTitle, af.section_id as sectionId, af.category_id as categoryId
               FROM locale_variants lv
               JOIN article_families af ON af.id = lv.family_id
               WHERE lv.id = @localeVariantId
               LIMIT 1`,
              { localeVariantId: requestedLocaleVariantId }
            )
          : null;
      ...
      const liveVariants = workspaceDb.all<PBIBatchGuaranteedEditLocaleVariant>(
        `SELECT lv.id as localeVariantId,
                lv.locale
         FROM locale_variants lv
    ...
    const guaranteedCreateArticles = dedupeGuaranteedCreateArticles(
      (Array.isArray(input.guaranteedCreateArticles) ? input.guaranteedCreateArticles : []).map((article) => {
        const placement = normalizePlacement(article);
        return {
          clientId: article.clientId?.trim() || randomUUID(),
          title: article.title,
    ...
    return {
      ...
      guaranteedEditFamilies: Array.from(guaranteedEditFamiliesById.values()).sort((left, right) =>
  ...
  private async hydratePBIBatchAnalysisState(
    workspace: WorkspaceRecord,
    workspaceDb: ReturnType<WorkspaceRepository['openWorkspaceDbWithRecovery']>,
    config: PBIBatchAnalysisConfig
  ): Promise<{
    analysisConfig: PBIBatchAnalysisConfig;
    guaranteedCreateConflicts: PBIBatchGuaranteedCreateConflict[];
  }> {
    const hydratedEditFamilies = await Promise.all(
      config.guaranteedEditFamilies.map(async (family) => {
        const familyRow = workspaceDb.get<{
          title: string;
          sectionId: string | null;
          categoryId: string | null;
        }>(
          `SELECT title, section_id as sectionId, category_id as categoryId
           FROM article_families
           WHERE id = @familyId
           LIMIT 1`,
          { familyId: family.familyId }
        ...
        const hydratedVariants = await Promise.all(
          family.resolvedLocaleVariants.map(async (variant) => {
            const latestRevision = workspaceDb.get<{
              revisionId: string;
              revisionType: RevisionState;
              updatedAtUtc: string;
              filePath: string;
    ...
    const guaranteedCreateConflicts = await Promise.all(
      config.guaranteedCreateArticles.map((article) =>
        this.detectGuaranteedCreateConflict(workspace.id, article)
      )
    );
    ...
    return {
      ...
      guaranteedCreateConflicts: guaranteedCreateConflicts.filter(
        (conflict): conflict is PBIBatchGuaranteedCreateConflict => Boolean(conflict)
      )
    };
  }

  private async detectGuaranteedCreateConflict(
    workspaceId: string,
    article: PBIBatchGuaranteedCreateArticle
  ): Promise<PBIBatchGuaranteedCreateConflict | null> {
    ...
    const strongMatches = dedupeGuaranteedCreateConflictMatches(
      searchResponse.results
        .filter((result) => isStrongGuaranteedCreateConflict(title, result.title))
        .map((result): PBIBatchGuaranteedCreateConflictMatch => ({
  ...
  async upsertKbScopeCatalogEntries(
    workspaceId: string,
    entries: KBScopeCatalogUpsertInput[]
  ): Promise<KBScopeCatalogRecord[]> {
    if (entries.length === 0) {
      return [];
    }
  ...
  async listKbScopeCatalogEntries(
    workspaceId: string,
    query: KBScopeCatalogQuery = {}
  ): Promise<KBScopeCatalogRecord[]> {
    const workspace = await this.getWorkspace(workspaceId);
  ...
  async listKbScopeOverrides(
    workspaceId: string,
    query: KBScopeOverrideQuery = {}
  ): Promise<KBScopeOverrideRecord[]> {
    const workspace = await this.getWorkspace(workspaceId);
    await this.ensureWorkspaceDb(workspace.path);
    const workspaceDb = this.openWorkspaceDbWithRecovery(path.join(workspace.path, '.meta', DEFAULT_DB_FILE));
    try {
  ...
  async resolveKbScopeDisplayNames(
    workspaceId: string,
    scopes: KBScopeDisplayNameInput[]
  ...
  private listArticleFamiliesForTaxonomyInDb(
    workspaceDb: ReturnType<WorkspaceRepository['openWorkspaceDbWithRecovery']>,
    workspaceId: string,
    familyIds?: string[]
  ): ArticleFamilyRecord[] {
  ...
  private getKbSectionParentCategoryInDb(
    workspaceDb: ReturnType<WorkspaceRepository['openWorkspaceDbWithRecovery']>,
    workspaceId: string,
    sectionId: string
  ): string | undefined {
    const row = workspaceDb.get<{
      overrideParentScopeId: string | null;
      catalogParentScopeId: string | null;
    }>(
      `SELECT (
  ...
  private isKbScopeCatalogEntryPresentInDb(
    workspaceDb: ReturnType<WorkspaceRepository['openWorkspaceDbWithRecovery']>,
    workspaceId: string,
    ...
    const row = workspaceDb.get<{ present: number }>(
      `SELECT 1 as present
         FROM kb_scope_catalog
        WHERE workspace_id = @workspaceId
  ...
  private resolveEffectiveArticleTaxonomyPlacementInDb(
    workspaceDb: ReturnType<WorkspaceRepository['openWorkspaceDbWithRecovery']>,
    workspaceId: string,
    family: ArticleFamilyRecord,
    options: {
      allowInference: boolean;
      indexDb?: SQLite;
    }
  ...
  private inferExistingArticleTaxonomyPlacementInDb(
    workspaceDb: ReturnType<WorkspaceRepository['openWorkspaceDbWithRecovery']>,
    workspaceId: string,
    familyId: string,
    indexDb: SQLite
  ): ArticleTaxonomyInferenceCandidate | null {
    ...
    const candidateFamilies = new Map(
      this.listArticleFamiliesForTaxonomyInDb(
        workspaceDb,
        workspaceId,
        response.results.map((result) => result.familyId)
      ).map((family) => [family.id, family])
    );

    const sectionVotes = new Map<string, {
      sectionId: string;
      categoryId: string;
      score: number;
      supporters: Set<string>;
      explicitLinkSupporters: Set<string>;
    }>();
    const categoryVotes = new Map<string, {
      categoryId: string;
      score: number;
      supporters: Set<string>;
      explicitLinkSupporters: Set<string>;
    ...
    for (const result of response.results) {
      ...
      if (candidateSectionId && candidateCategoryId) {
        const sectionVote = sectionVotes.get(candidateSectionId) ?? {
          ...
          explicitLinkSupporters: new Set<string>()
        };
    ...
    const rankedSections = Array.from(sectionVotes.values()).sort((left, right) => (
      right.score - left.score
      || right.supporters.size - left.supporters.size
      || right.explicitLinkSupporters.size - left.explicitLinkSupporters.size
      || left.sectionId.localeCompare(right.sectionId)
    ));
    const rankedCategories = Array.from(categoryVotes.values()).sort((left, right) => (
      right.score - left.score
      || right.supporters.size - left.supporters.size
      || right.explicitLinkSupporters.size - left.explicitLinkSupporters.size
    ...
    if (
      ...
      return {
        ...
        confidence: normalizeArticleTaxonomyConfidence(
          0.5
          + Math.min(0.25, topSection.score / 5)
          + Math.min(0.12, topSection.supporters.size * 0.04)
          + Math.min(0.06, topSection.explicitLinkSupporters.size * 0.03)
        ) ?? 0.82,
    ...
    if (
      ...
      return {
        ...
        confidence: normalizeArticleTaxonomyConfidence(
          ...
          + Math.min(0.04, topCategory.explicitLinkSupporters.size * 0.02)
        ) ?? 0.76,
  ...
  private applyArticleTaxonomyResolutionInDb(
    workspaceDb: ReturnType<WorkspaceRepository['openWorkspaceDbWithRecovery']>,
    workspaceId: string,
    family: ArticleFamilyRecord,
    resolution: ArticleTaxonomyResolution
  ): { family: ArticleFamilyRecord; changed: boolean } {
    const next: ArticleFamilyRecord = {
      ...family,
  ...
  async getArticleFamilyByExternalKey(workspaceId: string, externalKey: string): Promise<ArticleFamilyRecord | null> {
    ...
    const workspaceDb = this.openWorkspaceDbWithRecovery(path.join(workspace.path, '.meta', DEFAULT_DB_FILE));
    try {
      const row = workspaceDb.get<ArticleFamilyDbRow>(
        `SELECT ${ARTICLE_FAMILY_SELECT_COLUMNS}
  ...
  async markDraftBranchesAsConflicted(workspaceId: string, localeVariantId: string): Promise<void> {
    const workspace = await this.getWorkspace(workspaceId);
    await this.ensureWorkspaceDb(workspace.path);
    const workspaceDb = this.openWorkspaceDbWithRecovery(path.join(workspace.path, '.meta', DEFAULT_DB_FILE));
    try {
      workspaceDb.run(
           ...
           AND state NOT IN (@conflictedState, @obsoleteState, @discardedState)`,
        {
          workspaceId,
          localeVariantId,
          state: DraftBranchStatus.CONFLICTED,
          conflictedState: DraftBranchStatus.CONFLICTED,
  ...
  async listDraftBranches(
    ...
    try {
      ...
      const obsoleteParams: Record<string, unknown> = {
        workspaceId,
        obsoleteState: DraftBranchStatus.OBSOLETE
      };
      ...
      clauses.push(`state != '${DraftBranchStatus.OBSOLETE}'`);
  ...
  async getMigrationHealth(workspaceId?: string): Promise<WorkspaceMigrationHealthReport> {
    ...
    try {
      ...
      for (const row of rows) {
        ...
        let batchAnalysisRepair: WorkspaceBatchAnalysisRepairSummary | undefined;
  ...
  private repairWorkspaceDb(dbPath: string) {
    const migrationResult = applyWorkspaceMigrations(dbPath);
  ...
  private hasBatchAnalysisRepairChanges(summary?: WorkspaceBatchAnalysisRepairSummary): boolean {
    return Boolean(
      summary
      && (
  ...
  private repairBatchAnalysisRolloutState(dbPath: string): WorkspaceBatchAnalysisRepairSummary {
    const db = openWorkspaceDatabase(dbPath);
    const summary: WorkspaceBatchAnalysisRepairSummary = {
  ...
  private ensurePBIBatchAnalysisConfigColumn(dbPath: string) {
    const db = openWorkspaceDatabase(dbPath);
    try {
      const columns = db.all<{ name: string }>(`PRAGMA table_info(pbi_batches)`).map((c) => c.name);
  ...
  private ensureAgentModelIdColumn(dbPath: string) {
    const db = openWorkspaceDatabase(dbPath);
    try {
      const columns = db.all<{ name: string }>(`PRAGMA table_info(workspace_settings)`).map((c) => c.name);
  ...
  private ensureAcpModelIdColumn(dbPath: string) {
    const db = openWorkspaceDatabase(dbPath);
    try {
      const columns = db.all<{ name: string }>(`PRAGMA table_info(workspace_settings)`).map((c) => c.name);
  ...
  private ensureAiRunsAgentModelIdColumn(dbPath: string) {
    const db = openWorkspaceDatabase(dbPath);
    try {
      const columns = db.all<{ name: string }>(`PRAGMA table_info(ai_runs)`).map((c) => c.name);
      if (!columns.includes('agent_model_id')) {
  ...
  private normalizeBatchAnalysisExecutionCounts(
    counts?: Partial<BatchAnalysisExecutionCounts> | null
  ...
  private parseBatchAnalysisExecutionCounts(payload?: string | null): BatchAnalysisExecutionCounts {
    if (!payload) {
      return this.normalizeBatchAnalysisExecutionCounts();
    }
    try {
      return this.normalizeBatchAnalysisExecutionCounts(JSON.parse(payload) as Partial<BatchAnalysisExecutionCounts>);
    } catch {
      return this.normalizeBatchAnalysisExecutionCounts();
    }
  }

  private getBatchAnalysisIterationRow(
    ...
    return db.get<{
      ...
      lastReviewVerdict: BatchAnalysisArtifactVerdict | null;
      outstandingDiscoveredWorkCount: number;
      executionCountsJson: string | null;
      startedAtUtc: string;
      endedAtUtc: string | null;
      createdAtUtc: string;
      updatedAtUtc: string;
  ...
  private mapBatchAnalysisIterationRow(
    row: NonNullable<ReturnType<WorkspaceRepository['getBatchAnalysisIterationRow']>>
  ...
  private getLatestBatchAnalysisIterationFromDb(
    db: ReturnType<typeof openWorkspaceDatabase>,
    workspaceId: string,
    batchId: string
  ): BatchAnalysisIterationRecord | null {
    const row = db.get<ReturnType<WorkspaceRepository['getBatchAnalysisIterationRow']>>(
      `SELECT id,
              workspace_id as workspaceId,
              batch_id as batchId,
              iteration,
  ...
  private getLatestBatchAnalysisPlanFromDb(
    db: ReturnType<typeof openWorkspaceDatabase>,
    workspaceId: string,
    batchId: string
  ): BatchAnalysisPlan | null {
    const row = db.get<{ id: string; payloadJson: string }>(
    ...
    try {
      const plan = JSON.parse(row.payloadJson) as BatchAnalysisPlan;
      const itemStatuses = new Map(
        db.all<{ planItemId: string; executionStatus: BatchPlanItem['executionStatus'] }>(
          `SELECT plan_item_id as planItemId, execution_status as executionStatus
             FROM batch_analysis_plan_items
            WHERE workspace_id = @workspaceId AND plan_id = @planId`,
          { workspaceId, planId: row.id }
        ).map((entry) => [entry.planItemId, entry.executionStatus])
      );
      return {
        ...plan,
  ...
  private getLatestApprovedBatchAnalysisPlanFromDb(
    db: ReturnType<typeof openWorkspaceDatabase>,
    workspaceId: string,
    batchId: string
  ): BatchAnalysisPlan | null {
    const rows = db.all<{ id: string; payloadJson: string }>(
      `SELECT id, payload_json as payloadJson
         FROM batch_analysis_plans
        WHERE workspace_id = @workspaceId AND batch_id = @batchId
        ORDER BY created_at DESC`,
  ...
  private getLatestBatchAnalysisReviewFromDb(
    db: ReturnType<typeof openWorkspaceDatabase>,
    workspaceId: string,
    batchId: string
  ): BatchPlanReview | null {
    const row = db.get<{ payloadJson: string }>(
      `SELECT json_object(
          'id', id,
    ...
    try {
      return JSON.parse(row.payloadJson) as BatchPlanReview;
    } catch {
      return null;
    }
  }

  private getLatestBatchAnalysisWorkerReportFromDb(
    db: ReturnType<typeof openWorkspaceDatabase>,
    workspaceId: string,
    batchId: string
  ): BatchWorkerExecutionReport | null {
    const row = db.get<{ payloadJson: string }>(
      `SELECT payload_json as payloadJson
    ...
    try {
      return JSON.parse(row.payloadJson) as BatchWorkerExecutionReport;
    } catch {
      return null;
    }
  }

  private getLatestBatchAnalysisWorkerReportForIterationFromDb(
    db: ReturnType<typeof openWorkspaceDatabase>,
    iterationId: string
  ): BatchWorkerExecutionReport | null {
    const row = db.get<{ payloadJson: string }>(
  ...
  private getLatestBatchAnalysisFinalReviewFromDb(
    db: ReturnType<typeof openWorkspaceDatabase>,
    workspaceId: string,
    batchId: string
  ): BatchFinalReview | null {
    const row = db.get<{ payloadJson: string }>(
      `SELECT json_object(
          'id', id,
          'workspaceId', workspace_id,
    ...
    try {
      return JSON.parse(row.payloadJson) as BatchFinalReview;
    } catch {
      return null;
    }
  }

  private mapBatchAnalysisStageEventRow(row: {
    id: string;
    workspaceId: string;
    batchId: string;
    iterationId: string;
    ...
    stage: BatchAnalysisStageStatus;
    role: BatchAnalysisIterationRecord['role'];
    eventType: BatchAnalysisStageEventRecord['eventType'];
    status: BatchAnalysisIterationStatus | null;
    summary: string | null;
    sessionId: string | null;
    agentModelId: string | null;
    approvedPlanId: string | null;
    ...
    let details: BatchAnalysisStageEventDetails | undefined;
    if (row.detailsJson) {
      try {
        const parsed = JSON.parse(row.detailsJson) as BatchAnalysisStageEventDetails;
  ...
  private getLatestBatchAnalysisStageEventFromDb(
    db: ReturnType<typeof openWorkspaceDatabase>,
    workspaceId: string,
    batchId: string
  ): BatchAnalysisStageEventRecord | null {
    const row = db.get<Parameters<WorkspaceRepository['mapBatchAnalysisStageEventRow']>[0]>(
      `SELECT id,
              workspace_id as workspaceId,
              batch_id as batchId,
              iteration_id as iterationId,
  ...
  private hydrateBatchAnalysisPlan(
    db: ReturnType<typeof openWorkspaceDatabase>,
    workspaceId: string,
    row: { id: string; payloadJson: string }
  ): BatchAnalysisPlan | null {
    if (!row.payloadJson) {
      return null;
    }
  ...
  private serializeBatchAnalysisPlanOpenQuestions(plan: BatchAnalysisPlan): string[] {
    ...
    const promptsFromLegacyOpenQuestions = (plan.openQuestions ?? [])
      ...
      .filter(Boolean)
      .filter((prompt) => {
        const matchingStructuredQuestion = structuredQuestionByKey.get(
          this.normalizeBatchAnalysisQuestionPromptKey(prompt)
        );
  ...
  private shouldPersistBatchAnalysisQuestionAsOpen(question: BatchAnalysisQuestion): boolean {
    return question.status === 'pending'
      && !question.answer?.trim()
      && Boolean(question.prompt?.trim());
  }

  private normalizeBatchAnalysisQuestionPromptKey(prompt: string | undefined): string {
  ...
  private listBatchAnalysisIterationsFromDb(
    ...
    const rows = db.all<ReturnType<WorkspaceRepository['getBatchAnalysisIterationRow']>>(
      `SELECT id,
              workspace_id as workspaceId,
              batch_id as batchId,
              iteration,
  ...
  private listBatchAnalysisPlansFromDb(
    db: ReturnType<typeof openWorkspaceDatabase>,
    workspaceId: string,
    batchId: string
  ): BatchAnalysisPlan[] {
  ...
  private listBatchAnalysisReviewsFromDb(
    db: ReturnType<typeof openWorkspaceDatabase>,
    workspaceId: string,
    batchId: string
  ): BatchPlanReview[] {
    ...
    return rows.flatMap((row) => {
      try {
        return row.payloadJson ? [JSON.parse(row.payloadJson) as BatchPlanReview] : [];
      } catch {
        return [];
      }
    });
  ...
  private listBatchAnalysisWorkerReportsFromDb(
    db: ReturnType<typeof openWorkspaceDatabase>,
    workspaceId: string,
    batchId: string
    ...
    return rows.flatMap((row) => {
      try {
        return row.payloadJson ? [JSON.parse(row.payloadJson) as BatchWorkerExecutionReport] : [];
      } catch {
        return [];
      }
    });
  ...
  private listBatchAnalysisDiscoveredWorkFromDb(
    db: ReturnType<typeof openWorkspaceDatabase>,
    workspaceId: string,
    batchId: string
  ...
  private listBatchAnalysisAmendmentsFromDb(
    ...
    return rows.flatMap((row) => {
      try {
        return row.payloadJson ? [JSON.parse(row.payloadJson) as BatchPlanAmendment] : [];
      } catch {
        return [];
      }
    });
  ...
  private listBatchAnalysisFinalReviewsFromDb(
    db: ReturnType<typeof openWorkspaceDatabase>,
    workspaceId: string,
    batchId: string
  ): BatchFinalReview[] {
    ...
    return rows.flatMap((row) => {
      try {
        return row.payloadJson ? [JSON.parse(row.payloadJson) as BatchFinalReview] : [];
      } catch {
        return [];
      }
      });
  ...
  private parseBatchAnalysisToolCalls(raw: string | null | undefined): AgentToolCallAudit[] {
    ...
    try {
      ...
      return Array.isArray(parsed) ? parsed as AgentToolCallAudit[] : [];
    } catch {
      return [];
    }
  }

  private parseBatchAnalysisRawOutput(raw: string | null | undefined): string[] {
    if (!raw) {
      return [];
    }
    try {
  ...
  private mapLegacyRunRowToPersistedRun(row: LegacyBatchAnalysisRunRow | undefined | null): PersistedAgentAnalysisRun | null {
    if (!row) {
      return null;
    }
    return {
  ...
  private mapStageRunRow(row: BatchAnalysisStageRunRow): BatchAnalysisStageRunRecord {
    return {
      id: row.id,
      workspaceId: row.workspaceId,
      batchId: row.batchId,
  ...
  private mapStageRunRowToPersistedRun(row: BatchAnalysisStageRunRow): PersistedAgentAnalysisRun {
  ...
  private parseStageRunMarker(
    message: string | null | undefined
  ): { stage: BatchAnalysisStageRunRecord['stage']; role: BatchAnalysisStageRunRecord['role'] } | null {
    if (!message) {
      return null;
    }
    ...
    const validRole: BatchAnalysisStageRunRecord['role'][] = [
      'planner',
      'plan-reviewer',
      'worker',
      'final-reviewer'
  ...
  private listBatchAnalysisStageRunsFromDb(
    db: ReturnType<typeof openWorkspaceDatabase>,
    workspaceId: string,
    batchId: string
  ): BatchAnalysisStageRunRecord[] {
    const rows = db.all<BatchAnalysisStageRunRow>(
      `SELECT id,
              workspace_id as workspaceId,
              batch_id as batchId,
              iteration_id as iterationId,
  ...
  private listBatchAnalysisRunsFromDb(
    db: ReturnType<typeof openWorkspaceDatabase>,
    workspaceId: string,
    batchId: string
  ): PersistedAgentAnalysisRun[] {
    const rows = db.all<LegacyBatchAnalysisRunRow>(
      `SELECT id,
              workspace_id as workspaceId,
              batch_id as batchId,
              session_id as sessionId,
  ...
  private listBatchAnalysisStageEventsFromDb(
    db: ReturnType<typeof openWorkspaceDatabase>,
    workspaceId: string,
    batchId: string,
    limit = 100
  ): BatchAnalysisStageEventRecord[] {
    const rows = db.all<Parameters<WorkspaceRepository['mapBatchAnalysisStageEventRow']>[0]>(
      `SELECT id,
              workspace_id as workspaceId,
              batch_id as batchId,
              iteration_id as iterationId,
  ...
  private backfillLegacyBatchAnalysisIterations(db: ReturnType<typeof openWorkspaceDatabase>): number {
    const batches = db.all<{
      workspaceId: string;
      batchId: string;
      totalRuns: number;
    }>(
      `SELECT ai_runs.workspace_id as workspaceId,
  ...
  private backfillLegacyBatchAnalysisStageRuns(db: ReturnType<typeof openWorkspaceDatabase>): number {
    const aiRunColumns = new Set(
      db.all<{ name: string }>(`PRAGMA table_info(ai_runs)`).map((column) => column.name)
    );
    const selectAgentModelId = aiRunColumns.has('agent_model_id')
      ? 'agent_model_id as agentModelId'
      : 'NULL as agentModelId';
    const runs = db.all<LegacyBatchAnalysisRunRow>(
      `SELECT id,
              workspace_id as workspaceId,
              batch_id as batchId,
              session_id as sessionId,
    ...
    for (const run of runs) {
      ...
      const iteration =
        ...
        ) ?? db.get<{ id: string; iteration: number }>(
          `SELECT id, iteration
             FROM batch_analysis_iterations
            WHERE workspace_id = @workspaceId AND batch_id = @batchId
            ORDER BY updated_at DESC
      ...
      const workerReport = db.get<{
        iterationId: string;
        iteration: number;
        stage: BatchAnalysisStageRunRecord['stage'];
        role: BatchAnalysisStageRunRecord['role'];
      }>(
        `SELECT iteration_id as iterationId,
                iteration,
                stage,
                role
           FROM batch_analysis_worker_reports
      ...
      const fallbackStage: BatchAnalysisStageRunRecord['stage'] =
        run.status === 'complete'
          ? 'approved'
          : run.status === 'canceled'
            ? 'canceled'
            : 'failed';
      const fallbackRole: BatchAnalysisStageRunRecord['role'] =
        run.status === 'complete' ? 'final-reviewer' : 'worker';
      ...
      const nextAttempt = db.get<{ nextAttempt: number }>(
        `SELECT COALESCE(MAX(attempt), 0) + 1 as nextAttempt
           FROM batch_analysis_stage_runs
          WHERE iteration_id = @iterationId AND stage = @stage AND role = @role`,
        {
  ...
  private backfillLegacyBatchAnalysisWorkerReports(db: ReturnType<typeof openWorkspaceDatabase>): number {
    const iterations = db.all<{
      id: string;
      workspaceId: string;
      batchId: string;
      iteration: number;
    ...
    for (const iteration of iterations) {
      db.run(
        ...
        )`,
        {
          ...
          blockerNotesJson: JSON.stringify(
            iteration.status === 'completed' ? [] : [iteration.summary ?? 'Legacy run did not complete successfully.']
          ),
  ...
  private normalizeBatchAnalysisIterations(db: ReturnType<typeof openWorkspaceDatabase>): number {
    const iterations = db.all<{
      id: string;
      workspaceId: string;
      batchId: string;
    ...
    let updated = 0;
    for (const iteration of iterations) {
      const nextApprovedPlanId = iteration.approvedPlanId ?? db.get<{ id: string }>(
        `SELECT id
           FROM batch_analysis_plans
      ...
      const nextLastReviewVerdict = iteration.lastReviewVerdict ?? db.get<{ verdict: BatchAnalysisArtifactVerdict }>(
        `SELECT verdict
           FROM batch_analysis_final_reviews
          WHERE iteration_id = @iterationId
          ORDER BY created_at DESC
          LIMIT 1`,
        { iterationId: iteration.id }
      )?.verdict ?? db.get<{ verdict: BatchAnalysisArtifactVerdict }>(
        `SELECT verdict
           FROM batch_analysis_reviews
          WHERE iteration_id = @iterationId
          ORDER BY created_at DESC
      ...
      const unresolvedDiscoveries = db.get<{ total: number }>(
        `SELECT COUNT(*) as total
           FROM batch_analysis_discovered_work
          WHERE iteration_id = @iterationId
            AND COALESCE(status, 'pending_review') NOT IN ('approved', 'rejected')`,
        { iterationId: iteration.id }
      )?.total ?? 0;
      ...
      const nextEndedAtUtc = iteration.endedAtUtc ?? (
        iteration.status === 'completed' || iteration.status === 'failed' || iteration.status === 'canceled' || iteration.status === 'needs_human_review'
          ? iteration.updatedAtUtc
          : null
      );
      const needsUpdate =
        nextApprovedPlanId !== iteration.approvedPlanId
  ...
  private backfillBatchAnalysisStageEvents(db: ReturnType<typeof openWorkspaceDatabase>): number {
    const iterations = db.all<{
      id: string;
      workspaceId: string;
      batchId: string;
    ...
    let inserted = 0;
    for (const iteration of iterations) {
      const existingCount = db.get<{ total: number }>(
        `SELECT COUNT(*) as total
           FROM batch_analysis_stage_events
          WHERE iteration_id = @iterationId`,
        { iterationId: iteration.id }
  ...
  private buildExecutionCountsFromWorkerReport(report: BatchWorkerExecutionReport): BatchAnalysisExecutionCounts {
    ...
    for (const item of report.executedItems) {
      if (item.action === 'create') counts.create += 1;
      else if (item.action === 'edit') counts.edit += 1;
      else if (item.action === 'retire') counts.retire += 1;
  ...
  private async markProposalTargetRetired(
    workspacePath: string,
    workspaceDb: ReturnType<WorkspaceRepository['openWorkspaceDbWithRecovery']>,
    proposal: ProposalReviewRecord
  ): Promise<ProposalDecisionMutationResult> {
    ...
    if (proposal.localeVariantId) {
      ...
      if (family?.familyId) {
        try {
          ...
          logger.warn('workspace-repository.markProposalTargetRetired stale-mark failed', {
            workspaceId: proposal.workspaceId,
            familyId: family.familyId,
            localeVariantId: proposal.localeVariantId,
            message: error instanceof Error ? error.message : String(error)
  ...
  private async purgeDraftBranches(
    workspacePath: string,
    workspaceDb: ReturnType<WorkspaceRepository['openWorkspaceDbWithRecovery']>,
    branches: Array<{ id: string; localeVariantId: string }>
  ): Promise<void> {
    for (const branch of branches) {
      const revisionRows = workspaceDb.all<{ id: string; filePath: string }>(
      ...
      const sessionIds = workspaceDb.all<{ id: string }>(
        `SELECT id
         FROM article_ai_sessions
         WHERE branch_id = @branchId`,
        { branchId: branch.id }
      ...
      if (sessionIds.length > 0) {
        ...
        const sessionParams = sessionIds.reduce<Record<string, string>>((acc, sessionId, index) => {
          acc[`sessionId${index}`] = sessionId;
          return acc;
        }, {});
        workspaceDb.run(
          `DELETE FROM article_ai_messages
      ...
      workspaceDb.run(
        `DELETE FROM article_ai_sessions
         WHERE branch_id = @branchId`,
        { branchId: branch.id }
      );
      ...
      if (revisionIds.length > 0) {
        ...
        const revisionParams = revisionIds.reduce<Record<string, string>>((acc, revisionId, index) => {
        ...
        workspaceDb.run(
          `DELETE FROM article_lineage
           WHERE predecessor_revision_id IN (${revisionPlaceholders})
              OR successor_revision_id IN (${revisionPlaceholders})`,
          revisionParams
        );
      }
      ...
      workspaceDb.run(
        `DELETE FROM draft_revision_commits
         WHERE branch_id = @branchId`,
        { branchId: branch.id }
      );
      workspaceDb.run(
      ...
      workspaceDb.run(
        `DELETE FROM draft_branches
         WHERE id = @branchId`,
        { branchId: branch.id }
      );

      for (const revision of revisionRows) {
        await fs.rm(resolveRevisionPath(workspacePath, revision.filePath), { force: true });
      }
      await fs.rm(path.join(workspacePath, 'drafts', branch.localeVariantId, branch.id), {
        recursive: true,
        force: true
      });
    }
  ...
  private async resolveProposalSourceHtml(
    ...
    const resolveLatestVariantRevision = (localeVariantId: string): { filePath: string } | null =>
    ...
    if (!revision && proposal.action !== ProposalAction.CREATE && proposal.familyId) {
  ...
  private hydrateProposalDisplayFields(
    ...
    const currentPlacement = familyId
      ...
      : undefined;
    const suggestedPlacement = this.hydrateProposalPlacementSuggestionInDb(
      workspaceDb,
  ...
  private getFamilyPlacementSummaryInDb(
    workspaceDb: ReturnType<WorkspaceRepository['openWorkspaceDbWithRecovery']>,
    workspaceId: string,
    familyId: string
  ...
  private resolvePlacementSummaryInDb(
    ...
    const labels = this.resolveKbScopeDisplayNamesInDb(workspaceDb, workspaceId, [
      ...(categoryId ? [{ scopeType: 'category' as const, scopeId: categoryId }] : []),
      ...(sectionId ? [{ scopeType: 'section' as const, scopeId: sectionId }] : [])
    ]);
    const labelsByKey = new Map(
      labels.map((label) => [`${label.scopeType}:${label.scopeId ?? ''}`, label.displayName])
    );
  ...
  private hydrateProposalPlacementSuggestionInDb(
    workspaceDb: ReturnType<WorkspaceRepository['openWorkspaceDbWithRecovery']>,
    workspaceId: string,
    placement?: ProposalPlacementSuggestion
  ): ProposalPlacementSuggestion | undefined {
    if (!placement) {
      return undefined;
    }
  ...
  private findOpenProposalByIdempotencyKey(
    workspaceDb: ReturnType<WorkspaceRepository['openWorkspaceDbWithRecovery']>,
    payload: {
      workspaceId: string;
      batchId: string;
    ...
    const rows = workspaceDb.all<{ id: string; queueOrder: number; metadataJson: string | null }>(`
      SELECT id,
             queue_order as queueOrder,
             metadata_json as metadataJson
      FROM proposals
  ...
  private enforceSingleActiveTemplatePerType(
    workspaceDb: ReturnType<typeof openWorkspaceDatabase>,
    workspaceId: string,
    options?: {
      templateType?: TemplatePackType;
    ...
    const rows = workspaceDb.all<Array<{
      id: string;
    ...
    const activeRowsByType = new Map<TemplatePackType, Array<{ id: string; updatedAtUtc: string }>>();
    for (const row of rows) {
      if (row.active !== 1) {
        continue;
      }
    ...
    const deactivateStatement = workspaceDb.prepare(
      `UPDATE template_packs
          SET active = 0
        WHERE workspace_id = @workspaceId AND id = @templatePackId`
    );
  ...
  private async resolveArticleAiTarget(
    ...
    const familyRow = workspaceDb.get<ArticleFamilyDbRow>(
      `SELECT ${ARTICLE_FAMILY_SELECT_COLUMNS}
       FROM article_families
       WHERE id = @familyId`,
      { familyId: variant.familyId }
...
function normalizeDraftBranchStatus(value: string | null | undefined, hasConflict = false): DraftBranchStatus {
  switch (value) {
    case DraftBranchStatus.READY_TO_PUBLISH:
      return hasConflict ? DraftBranchStatus.CONFLICTED : DraftBranchStatus.READY_TO_PUBLISH;
...
function normalizeTemplatePackType(value: string | null | undefined): TemplatePackType {
  switch (value) {
    ...
    case TemplatePackType.PROPOSAL_CREATION:
...
function createDefaultPBIBatchAnalysisConfig(updatedAtUtc = new Date().toISOString()): PBIBatchAnalysisConfig {
  return {
    version: PBI_BATCH_ANALYSIS_CONFIG_VERSION,
    updatedAtUtc,
...
function normalizePBIBatchAnalysisConfig(value: unknown): PBIBatchAnalysisConfig {
  ...
  const guaranteedEditFamilies = Array.isArray(record.guaranteedEditFamilies)
    ? record.guaranteedEditFamilies.flatMap((family): PBIBatchGuaranteedEditFamily[] => {
        if (!family || typeof family !== 'object') {
          return [];
        }
        const candidate = family as Record<string, unknown>;
        ...
        const resolvedLocaleVariants = Array.isArray(candidate.resolvedLocaleVariants)
          ? candidate.resolvedLocaleVariants.flatMap((variant): PBIBatchGuaranteedEditLocaleVariant[] => {
              if (!variant || typeof variant !== 'object') {
                return [];
              }
              const resolvedVariant = variant as Record<string, unknown>;
              ...
              return [{
                ...
                revisionState: extractString(resolvedVariant.revisionState) as RevisionState | undefined,
  ...
  const guaranteedCreateArticles = dedupeGuaranteedCreateArticles(
    Array.isArray(record.guaranteedCreateArticles)
      ? record.guaranteedCreateArticles.flatMap((article): PBIBatchGuaranteedCreateArticle[] => {
          ...
          return [{
            clientId: extractString(candidate.clientId) ?? randomUUID(),
            title,
...
function normalizeOptionalPromptText(value?: string | null): string | undefined {
...
function dedupeGuaranteedCreateArticles(
  articles: PBIBatchGuaranteedCreateArticle[]
): PBIBatchGuaranteedCreateArticle[] {
  const seen = new Set<string>();
  const deduped: PBIBatchGuaranteedCreateArticle[] = [];
...
function dedupeGuaranteedCreateConflictMatches(
  matches: PBIBatchGuaranteedCreateConflictMatch[]
): PBIBatchGuaranteedCreateConflictMatch[] {
  const seen = new Set<string>();
  const deduped: PBIBatchGuaranteedCreateConflictMatch[] = [];
...
function normalizeGuaranteedTargetTitle(value: string): string {
  return value
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, ' ')
...
function tokenizeGuaranteedTargetTitle(value: string): string[] {
...
function isStrongGuaranteedCreateConflict(requestedTitle: string, existingTitle: string): boolean {
...
function normalizeSearchIdList(values?: string[]): string[] {
  return Array.from(
    new Set(
      (values ?? [])
        .map((value) => value.trim())
...
function normalizeKbScopeType(value: KBScopeType | string): KBScopeType {
...
function normalizeRequiredKbScopeId(value: string | null | undefined, fieldName: string): string {
...
function normalizeRequiredKbScopeDisplayName(value: string | null | undefined): string {
  const normalized = value?.trim();
  if (!normalized) {
...
function normalizeRequiredKbScopeSource(value: string | null | undefined): string {
  const normalized = value?.trim();
  if (!normalized) {
...
function normalizeArticleTaxonomySource(value: ArticleTaxonomySource | string | null | undefined): ArticleTaxonomySource {
  if (typeof value === 'string' && ARTICLE_TAXONOMY_SOURCE_VALUES.includes(value as ArticleTaxonomySource)) {
    return value as ArticleTaxonomySource;
  }
  return 'none';
}

function normalizeArticleTaxonomyConfidence(value: number | null | undefined): number | undefined {
  if (typeof value !== 'number' || !Number.isFinite(value)) {
    return undefined;
  }
...
function mapArticleFamilyRow(row: ArticleFamilyDbRow): ArticleFamilyRecord {
  return {
    id: row.id,
    workspaceId: row.workspaceId,
...
function buildFallbackKbScopeDisplayName(scopeType: 'section' | 'category', scopeId?: string | null): string {
  const normalizedScopeId = normalizeFeatureMapScopeId(scopeId);
  if (normalizedScopeId) {
...
function deriveFeatureMapScopeName(scopeType: 'section' | 'category', scopeId?: string | null): string {
...
function buildFeatureMapBridges(input: {
  ...
  const bridges = new Map<string, {
    ...
    targetScopeType: 'section' | 'category';
...
function clampGraphLimit(value: number | undefined): number {
  if (!Number.isFinite(value)) {
    return 36;
  }
...
function limitArticleRelationGraphEdges(
  relations: ArticleRelationRecord[],
  limitNodes: number,
  seedFamilyId?: string
): ArticleRelationRecord[] {
  const selected: ArticleRelationRecord[] = [];
  const nodeIds = new Set<string>(seedFamilyId ? [seedFamilyId] : []);
...
function shouldReevaluateFromRelationFeedback(feedbackType: ArticleRelationFeedbackType): boolean {
  return feedbackType === ArticleRelationFeedbackType.ADD
    || feedbackType === ArticleRelationFeedbackType.REMOVE
    || feedbackType === ArticleRelationFeedbackType.MISSED
...
function buildRelationCoverageInClause(
  prefix: string,
  values: string[],
