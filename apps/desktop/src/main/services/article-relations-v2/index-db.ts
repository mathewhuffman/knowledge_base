import { openWorkspaceDatabase, type SQLite } from '@kb-vault/db';
import type { RelationDocument } from '@kb-vault/shared-types';
import {
  ARTICLE_RELATIONS_V2_ENGINE_VERSION,
  dedupeArticleRelationsV2ExplicitLinks,
  getArticleRelationsV2ExplicitLinkKey,
  type ArticleRelationsV2IndexDocumentRow,
  type ArticleRelationsV2IndexDocumentState,
  type ArticleRelationsV2IndexStats
} from './types';
...
export class ArticleRelationsV2IndexDb {
  constructor(private readonly dbPath: string) {}

  open(): SQLite {
    const db = openWorkspaceDatabase(this.dbPath);
    this.ensureSchema(db);
    return db;
  }

  ensureSchema(db: SQLite): void {
    db.exec(INDEX_SCHEMA);
  }

  clearAll(db: SQLite): void {
  ...
  listDocumentStates(db: SQLite): ArticleRelationsV2IndexDocumentState[] {
    return db.all<ArticleRelationsV2IndexDocumentState>(
  ...
  deleteDocuments(db: SQLite, localeVariantIds: string[]): void {
    for (const localeVariantId of localeVariantIds) {
      this.deleteDocument(db, localeVariantId);
    }
  }

  replaceDocument(db: SQLite, document: RelationDocument): void {
    ...
    for (const link of dedupeArticleRelationsV2ExplicitLinks(document.explicitLinks)) {
      const linkKey = getArticleRelationsV2ExplicitLinkKey(document.localeVariantId, link);
      try {
        db.run(
        ...
        throw new Error(
          `Failed to index explicit link for family ${document.familyId}, locale variant ${document.localeVariantId}, href ${link.href}: ${errorMessage}`,
          { cause: error }
        );
      }
    }
  ...
  updateMetadata(db: SQLite, workspaceId: string, lastBuiltAtUtc: string): ArticleRelationsV2IndexStats {
    const counts = db.get<{
  ...
  getStats(db: SQLite, workspaceId: string): ArticleRelationsV2IndexStats {
    const row = db.get<{
      workspaceId: string;
  ...
  private deleteDocument(db: SQLite, localeVariantId: string): void {
...
function toIndexDocumentRow(document: RelationDocument): ArticleRelationsV2IndexDocumentRow {
  return {
    workspaceId: document.workspaceId,
    familyId: document.familyId,
    localeVariantId: document.localeVariantId,
